...
// caffe2/caffe2/core/observer.h

#ifndef BUBBLEFS_UTILS_CAFFE2_OBSERVER_H_
#define BUBBLEFS_UTILS_CAFFE2_OBSERVER_H_
...
class ObserverBase {
 public:
  ...
  virtual std::unique_ptr<ObserverBase<T>> copy(T* subject) {
    PRINTF_WARN("copy() is not implemented and nullptr will be returned.\n");
    return nullptr;
  }
...
class Observable {
 public:
  ...
  const Observer* AttachObserver(std::unique_ptr<Observer> observer) {
    PANIC_ENFORCE(observer, "Couldn't attach a null observer.");
...
#endif // BUBBLEFS_UTILS_CAFFE2_OBSERVER_H_