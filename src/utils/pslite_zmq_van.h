...
class ZMQVan : public Van {
 ...
 protected:
  void Start() override {
    ...
    PANIC_ENFORCE(context_ != NULL, "create 0mq context failed");
  ...
  void Stop() override {
    ...
    PANIC_ENFORCE(rc == 0 || errno == ETERM, "rc == 0 || errno == ETERM is not true");
    PANIC_ENFORCE_EQ(zmq_close(receiver_), 0);
    for (auto& it : senders_) {
      int rc = zmq_setsockopt(it.second, ZMQ_LINGER, &linger, sizeof(linger));
      PANIC_ENFORCE(rc == 0 || errno == ETERM, "rc == 0 || errno == ETERM is not true");
      PANIC_ENFORCE_EQ(zmq_close(it.second), 0);
    }
  ...
  int Bind(const Node& node, int max_retry) override {
    ...
    PANIC_ENFORCE(receiver_ != NULL, "create receiver socket failed: %s", zmq_strerror(errno));
  ...
  void Connect(const Node& node) override {
    PANIC_ENFORCE_NE(node.id, node.kEmpty);
    PANIC_ENFORCE_NE(node.port, node.kEmpty);
    PANIC_ENFORCE(node.hostname.size(), "hostname is empty");
    int id = node.id;
    ...
    if (zmq_connect(sender, addr.c_str()) != 0) {
      PANIC("connect to %s failed: %s", addr.c_str(), zmq_strerror(errno));
    }
    senders_[id] = sender;
  }
  ...
  int SendMsg(const Message& msg) override {
    ...
    PANIC_ENFORCE_NE(id, Meta::kEmpty);
    auto it = senders_.find(id);
    if (it == senders_.end()) {
      PRINTF_WARN("there is no socket to node %d\n", id);
      return -1;
    }
    ...
    while (true) {
      ...
      PRINTF_WARN("failed to send message to node [%d] errno: %d %s\n",
  ...
  int RecvMsg(Message* msg) override {
    ...
    for (int i = 0; ; ++i) {
      ...
      PANIC_ENFORCE(zmq_msg_init(zmsg) == 0, "%s", zmq_strerror(errno));
      while (true) {
        if (zmq_msg_recv(zmsg, receiver_, 0) != -1) break;
        if (errno == EINTR) continue;
        PRINTF_WARN("failed to send message, errno: %d %s\n",
      ...
      if (i == 0) {
        ...
        PANIC_ENFORCE(zmq_msg_more(zmsg), "zmq_msg_more(zmsg) no more");
        zmq_msg_close(zmsg);
        delete zmsg;
      } else if (i == 1) {
        // task
...
#endif  // BUBBLEFS_UTILS_PSLITE_ZMQ_VAN_H_