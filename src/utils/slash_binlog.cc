...
// slash/slash/src/slash_binlog_impl.h
// slash/slash/src/slash_binlog_impl.cc
...
Status BinlogImpl::SetProducerStatus(uint32_t pro_num, uint64_t pro_offset) {
  ...
  if (FileExists(init_profile.c_str())) {
    DeleteFile(init_profile.c_str());
  }

  std::string profile = NewFileName(path_ + kBinlogPrefix, pro_num);
  if (FileExists(profile.c_str())) {
