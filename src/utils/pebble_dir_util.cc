...
int DirUtil::MakeDir(const std::string& path) {
    if (path.empty()) {
        _PEBBLE_LOG_LAST_ERROR("param path is NULL");
        return -1;
    }
    ...
    if (mkdir(path.c_str(), 0755) != 0) {
        _PEBBLE_LOG_LAST_ERROR("mkdir %s failed(%s)", path.c_str(), strerror(errno));
        return -1;
    }
...
int DirUtil::MakeDirP(const std::string& path) {
    ...
    if (path.size() > PATH_MAX) {
        _PEBBLE_LOG_LAST_ERROR("path length %ld > PATH_MAX(%d)", path.size(), PATH_MAX);
        return -1;
    }
