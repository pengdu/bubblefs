...
// tera/src/zk/zk_util.cc

#include "utils/tera_zk_util.h"
#include <string.h>
#include <string>
#include "platform/base_error.h"
#include "utils/bdcom_file_path.h"
...
int32_t ZooKeeperUtil::GetSequenceNo(const std::string& name) {
    ...
    if (*seq_str != '\0') {
        ...
        if (*seq_end_ptr == '\0' && seq_no > 0) {
            ...
            PRINTF_ERROR("name [%s] not end in 10 digit", name.c_str());
            return -1;
        }
    } else {
        return 0;
...
bool FakeZkUtil::WriteNode(const std::string& name, const std::string& value) {
    mybdcom::FileStream node_file;
    if (!node_file.Open(name, mybdcom::FILE_WRITE)) {
        return false;
    }
...
bool FakeZkUtil::ReadNode(const std::string& name, std::string* value) {
    mybdcom::FileStream node_file;
    if (!node_file.Open(name, mybdcom::FILE_READ)) {
        PRINTF_ERROR("fail to open node file: %s\n", name.c_str());
        return false;
    }
    if (node_file.ReadLine(value) < 0) {
        PRINTF_ERROR("fail to read node file: %s\n", name.c_str());
        return false;
    }
    node_file.Close();
    return true;
...
bool FakeZkUtil::ListNodes(const std::string& path,
    ...
    if (!mybdcom::ListCurrentDir(path, values)) {
        return false;
    }
    return true;
}
