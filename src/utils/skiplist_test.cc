...
class ConcurrentTest {
 private:
  ...
  static uint64_t HashNumbers(uint64_t k, uint64_t g) {
    uint64_t data[2] = { k, g };
    return Hash32(reinterpret_cast<char*>(data), sizeof(data), 0);
  }
  ...
  static Key RandomTarget(random::Random* rnd) {
    switch (rnd->Next() % 10) {
      case 0:
        // Seek to beginning
        return MakeKey(0, 0);
 ...
 public:
  ...
  void WriteStep(random::Random* rnd) {
  ...
  void ReadStep(random::Random* rnd) {
...
TEST_F(SkipTest, ConcurrentWithoutThreads) {
  ConcurrentTest test;
  random::Random rnd(testing::RandomSeed());
...
static void RunConcurrent(int run) {
  const int seed = testing::RandomSeed() + (run * 100);
  random::Random rnd(seed);
  const int N = 1000;
