...
// apollo/modules/common/util/file.h

/**
 * @file
 */

#ifndef BUBBLEFS_UTILS_APOLLO_FILE_H_
#define BUBBLEFS_UTILS_APOLLO_FILE_H_
...
bool SetProtoToASCIIFile(const MessageType &message,
  ...
  if (fd < 0) {
    PRINTF_ERROR("Unable to open file %s to write.\n", file_name.c_str());
    return false;
  }
...
bool GetProtoFromASCIIFile(const std::string &file_name, MessageType *message) {
  ...
  if (file_descriptor < 0) {
    PRINTF_ERROR("Failed to open file %s in text mode.\n", file_name.c_str());
    // Failed to open;
    return false;
  }
  ...
  if (!success) {
    PANIC("Failed to parse file %s as text proto.", file_name.c_str());
  }
  delete input;
  close(file_descriptor);
  return success;
...
bool GetProtoFromBinaryFile(const std::string &file_name,
  ...
  if (!input.good()) {
    PRINTF_ERROR("Failed to open file %s in binary mode.\n", file_name.c_str());
    return false;
  }
  if (!message->ParseFromIstream(&input)) {
    PRINTF_ERROR("Failed to parse file %s as binary proto.\n", file_name.c_str());
    return false;
  }
  return true;
}
...
#endif  // BUBBLEFS_UTILS_APOLLO_FILE_H_