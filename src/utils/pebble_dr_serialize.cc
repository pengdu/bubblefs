...
// Pebble/src/framework/dr/serialize.cpp

#include "utils/pebble_dr_serialize.h"
#include <cstring>
#include <iostream>
#include "utils/pebble_dr_binary_protocol.h"
...
static int find_field(const std::vector<int16_t> &path, size_t path_idx,
    dr::protocol::TProtocol &protocol, uint32_t xfer_base,
    dr::protocol::TType &found_field_type, uint32_t &found_field_pos) {
    if (path_idx < path.size()) {
        int16_t fid_want = path[path_idx];

        uint32_t xfer = 0;
        std::string fname;
        dr::protocol::TType ftype = dr::protocol::T_NULL;
        int16_t fid = -1;

        xfer = protocol.readStructBegin(fname);
        while (true) {
            xfer += protocol.readFieldBegin(fname, ftype, fid);
            if (ftype == dr::protocol::T_STOP) {
                break;
            }
            if (fid == fid_want) {
...
static const size_t fix_type_len[dr::protocol::T_UTF16 + 1] = {
