...
// Pebble/src/framework/naming.h

#ifndef BUBBLEFS_UTILS_PEBBLE_COMMON_NAMING_H_
#define BUBBLEFS_UTILS_PEBBLE_COMMON_NAMING_H_

#include <string>
#include <unordered_map>
#include <vector>
#include "platform/pebble_error.h"
#include "platform/types.h"

namespace bubblefs {
namespace mypebble {

typedef std::function<void(const std::vector<std::string>& urls)> WatchFunc;
/// @brief 异步注册接口的执行结果回调
/// @param rc 执行结果返回值
typedef std::function<void(int rc)> CbReturnCode;
/// @brief 异步查询接口的执行结果回调
/// @param rc 执行结果返回值
/// @param values 查询结果(url列表)
typedef std::function<void(int rc, const std::vector<std::string>& urls)> CbReturnValue;
...
/// @return 0成功，非0失败
int32_t SetNamingFactory(int32_t type, const std::shared_ptr<NamingFactory>& factory);

/// @brief 获取指定类型的名字服务工厂
/// @param type 名字服务的类型
/// @return 名字服务的工厂实例，为NULL时说明未set这种类型的工厂
std::shared_ptr<NamingFactory> GetNamingFactory(int32_t type);


} // namespace mypebble
} // namespace bubblefs

#endif // BUBBLEFS_UTILS_PEBBLE_COMMON_NAMING_H_