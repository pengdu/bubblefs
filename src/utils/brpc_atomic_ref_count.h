...
// brpc/src/butil/atomic_ref_count.h

#ifndef BUBBLEFS_UTILS_BRPC_ATOMIC_REF_COUNT_H_
#define BUBBLEFS_UTILS_BRPC_ATOMIC_REF_COUNT_H_
...
inline void AtomicRefCountIncN(volatile AtomicRefCount *ptr,
  ...
  base::NoBarrier_AtomicIncrement(ptr, increment);
}
...
inline bool AtomicRefCountDecN(volatile AtomicRefCount *ptr,
  ...
  bool res = (base::Barrier_AtomicIncrement(ptr, -decrement) != 0);
  if (!res) {
...
inline bool AtomicRefCountIsOne(volatile AtomicRefCount *ptr) {
  bool res = (base::Acquire_Load(ptr) == 1);
  if (res) {
    ANNOTATE_HAPPENS_AFTER(ptr);
  }
  return res;
...
inline bool AtomicRefCountIsZero(volatile AtomicRefCount *ptr) {
  bool res = (base::Acquire_Load(ptr) == 0);
  if (res) {
    ANNOTATE_HAPPENS_AFTER(ptr);
  }
  return res;
