...
// dmlc-core/src/config.cc

#include <sstream>
#include <exception>

#include "platform/base_error.h"
#include "utils/dmlc_config.h"
...
void Config::LoadFromStream(istream& is) {
  ...
  try {
    while ( true ) {
      ...
      if (eqop.buf != "=") {
        PRINTF_ERROR("Parsing error: expect format \"k = v\"; but got \"%s %s %s\"\n",
                     key.buf.c_str(), eqop.buf.c_str(), value.buf.c_str());
      }
      Insert(key.buf, value.buf, value.is_string);
    }
  } catch(TokenizeError& err) {
    PRINTF_ERROR("Tokenize error: %s\n", err.what());
  }
}

const string& Config::GetParam(const string& key) const {
  PANIC_ENFORCE(config_map_.find(key) != config_map_.end(),
               "key %s not found in configure", key.c_str());
