...
TEST_P(CacheTest, EvictEmptyCache) {
  ...
  ASSERT_TRUE(cache->Insert("foo", nullptr, 10, dumbDeleter).ok());
}
...
TEST_P(CacheTest, EraseFromDeleter) {
  ...
  ASSERT_TRUE(cache->Insert("foo", nullptr, 1, dumbDeleter).ok());
  ASSERT_TRUE(cache->Insert("bar", cache.get(), 1, eraseDeleter).ok());
  cache->Erase("bar");
...
std::shared_ptr<Cache> (*new_clock_cache_func)(size_t, int, bool) = NewClockCache;
