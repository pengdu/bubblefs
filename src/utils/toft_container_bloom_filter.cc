...
#include "utils/toft_container_bloom_filter.h"
...
void BloomFilter::Initialize(size_t element_count, double false_positive_prob)
{
    ...
    if (num_bits == 0)
    {
        PANIC("Num elements=%u false_positive_prob=%g",
              (unsigned)element_count, false_positive_prob);
    }

    if (num_bytes > ~size_t(0))
    {
        PANIC("Bitmap too large, size=%u, exceed size_t limitation",
              (unsigned)num_bytes);
    }
...
void BloomFilter::CheckBitmapSize(size_t byte_size)
{
    ...
    {
        PANIC("Bitmap too large, bit count=%u, exceed 4Gbits(512 Mbytes)",
               (unsigned)num_bits);
    }
}
