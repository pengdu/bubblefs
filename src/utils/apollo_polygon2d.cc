...
// apollo/modules/common/math/polygon2d.cc

#include "utils/apollo_polygon2d.h"
...
#include "utils/apollo_math_utils.h"
...
double Polygon2d::DistanceTo(const Vec2d &point) const {
  PANIC_ENFORCE_GE(points_.size(), 3);
  if (IsPointIn(point)) {
    return 0.0;
  }
...
double Polygon2d::DistanceTo(const Polygon2d &polygon) const {
  PANIC_ENFORCE_GE(points_.size(), 3);
  PANIC_ENFORCE_GE(polygon.num_points(), 3);
...
void Polygon2d::BuildFromPoints() {
  ...
  PANIC_ENFORCE_GE(num_points_, 3);
  ...
  area_ /= 2.0;
  PANIC_ENFORCE_GT(area_, kMathEpsilon);
...
bool Polygon2d::ComputeOverlap(const Polygon2d &other_polygon,
  ...
  PANIC_ENFORCE_GE(points_.size(), 3);
  PANIC_ENFORCE_NE(overlap_polygon, NULL);
  PANIC_ENFORCE(is_convex_ && other_polygon.is_convex(), "one of these polygons is not convex");
...
bool Polygon2d::GetOverlap(const LineSegment2d &line_segment,
  ...
  PANIC_ENFORCE_GE(points_.size(), 3);
  PANIC_ENFORCE_NE(first, NULL);
  PANIC_ENFORCE_NE(last, NULL);
...
Box2d Polygon2d::MinAreaBoundingBox() const {
  ...
  if (!is_convex_) {
    ...
    PANIC_ENFORCE(convex_polygon.is_convex(), "convex_polygon is not convex");
...
Polygon2d Polygon2d::ExpandByDistance(const double distance) const {
  ...
  PANIC_ENFORCE(ComputeConvexHull(points, &new_polygon), "ComputeConvexHull fail");
  return new_polygon;
}

std::string Polygon2d::DebugString() const {
  std::ostringstream oss;
  oss << "polygon2d (  num_points = " << num_points_
