...
#ifndef BUBBLEFS_UTILS_DMLC_SERIALIZER_H_
#define BUBBLEFS_UTILS_DMLC_SERIALIZER_H_
...
struct Handler {
   ...
   */
  inline static void Write(Stream *strm, const T &data) {
    IfThenElse<mydmlc::is_pod<T>::value,
               PODHandler<T>,
               IfThenElse<mydmlc::has_saveload<T>::value,
   ...
   */
  inline static bool Read(Stream *strm, T *data) {
    return IfThenElse<mydmlc::is_pod<T>::value,
                      PODHandler<T>,
                      IfThenElse<mydmlc::has_saveload<T>::value,
                                 SaveLoadClassHandler<T>,
                                 UndefinedSerializerFor<T>, T>,
...
struct Handler<std::pair<TA, TB> > {
  inline static void Write(Stream *strm, const std::pair<TA, TB> &data) {
    IfThenElse<mydmlc::is_pod<TA>::value && mydmlc::is_pod<TB>::value,
  ...
  inline static bool Read(Stream *strm, std::pair<TA, TB> *data) {
    return IfThenElse<mydmlc::is_pod<TA>::value && mydmlc::is_pod<TB>::value,
                      PODHandler<std::pair<TA, TB> >,
                      PairHandler<TA, TB>,
                      std::pair<TA, TB> >
    ::Read(strm, data);
...
#endif  // BUBBLEFS_UTILS_DMLC_SERIALIZER_H_