...
#ifndef BUBBLEFS_UTILS_NNVM_OP_H_
#define BUBBLEFS_UTILS_NNVM_OP_H_
...
class Op {
 ...
 private:
  ...
  friend class mydmlc::Registry<Op>;
...
#define NNVM_REGISTER_VAR_DEF(OpName)                                   \
  static ATTRIBUTE_UNUSED ::bubblefs::mynnvm::Op & __make_ ## NnvmOp ## _ ## OpName

#define NNVM_REGISTER_GVAR_DEF(TagName)                                     \
  static ATTRIBUTE_UNUSED ::bubblefs::mynnvm::OpGroup __make_ ## NnvmOpGroup ## _ ## TagName
...
#define NNVM_REGISTER_OP(OpName)                                     \
  DMLC_STR_CONCAT(NNVM_REGISTER_VAR_DEF(OpName), __COUNTER__) =         \
      ::bubblefs::mydmlc::Registry<::nnvm::Op>::Get()->__REGISTER_OR_GET__(#OpName)
...
#define NNVM_REGISTER_OP_GROUP(GroupName)                               \
  DMLC_STR_CONCAT(NNVM_REGISTER_GVAR_DEF(GroupName), __COUNTER__) =     \
      ::bubblefs::mynnvm::OpGroup {#GroupName}
...
inline const OpMap<ValueType>& Op::GetAttr(const std::string& key) {
  ...
  return mynnvm::get<OpMap<ValueType> >(*ref);
}
...
inline Op& Op::set_attr(  // NOLINT(*)
    ...
    const ValueType& value,
    int plevel) {
  PANIC_ENFORCE_GT(plevel, 0); // "plevel in set_attr must be greater than 0";
  ...
  UpdateAttrMap(attr_name,
      ...
      std::vector<std::pair<ValueType, int> >& vec =
          mynnvm::get<OpMap<ValueType> >(*pmap).data_;
...
inline const ValueType& OpMap<ValueType>::operator[](const Op* op) const {
  PANIC_ENFORCE(op != nullptr, "");
  const uint32_t idx = op->index_;
  PANIC_ENFORCE(idx < data_.size() && data_[idx].second,
...
#endif  // BUBBLEFS_UTILS_NNVM_OP_H_