...
#include "platform/pdlfs_posix_fio.h"
#endif
...
#if VERBOSE >= 2
  Verbose(__PDLFS_LOG_ARGS__, 2, "fio.posix.root -> %s", root.c_str());
#endif
  return root;
}
...
#if VERBOSE >= 1
  Verbose(__PDLFS_LOG_ARGS__, 1, "fio.name -> %s", fio_name.c_str());
  Verbose(__PDLFS_LOG_ARGS__, 1, "fio.conf -> %s", fio_conf.c_str());
#endif
#if defined(PDLFS_RADOS)
  if (fio_name == "rados") {
