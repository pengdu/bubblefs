...
namespace mydmlc {
// enable registry
DMLC_REGISTRY_ENABLE(mynnvm::Op);
}  // namespace mydmlc
}  // namespace bubblefs
...
Op& Op::add_alias(const std::string& alias) {  // NOLINT(*)
  mydmlc::Registry<Op>::Get()->AddAlias(this->name, alias);
  return *this;
}

// find operator by name
const Op* Op::Get(const std::string& name) {
  const Op* op = mydmlc::Registry<Op>::Find(name);
  PANIC_ENFORCE(op != nullptr, ""); // "Operator " << name << " is not registered";
  return op;
}
...
void Op::AddGroupTrigger(const std::string& group_name,
  ...
  for (const Op* op : mydmlc::Registry<Op>::List()) {
