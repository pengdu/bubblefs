...
int32_t IRpc::OnMessage(int64_t handle, const uint8_t* msg,
    ...
    if (NULL == msg || 0 == msg_len) {
        PEBBLE_PLOG_ERROR_N_EVERY_SECOND(1, "param invalid: buff = %p, buff_len = %u", msg, msg_len);
    ...
    if (head_len < 0) {
        PEBBLE_PLOG_ERROR_N_EVERY_SECOND(1, "HeadDecode failed(%d).", head_len);
        return kRPC_DECODE_FAILED;
    }

    if (head_len > static_cast<int32_t>(msg_len)) {
        PEBBLE_PLOG_ERROR_N_EVERY_SECOND(1, "head_len(%d) > buff_len(%u)", head_len, msg_len);
        return kRPC_DECODE_FAILED;
    }
    ...
    switch (head.m_message_type) {
        ...
        default:
            PEBBLE_PLOG_ERROR_N_EVERY_SECOND(1, "rpc msg type error(%d)", head.m_message_type);
            break;
    }
...
int32_t IRpc::AddOnRequestFunction(const std::string& name, const OnRpcRequest& on_request) {
    if (name.empty() || !on_request) {
        PEBBLE_PLOG_ERROR("param invalid: name = %s, !on_request = %u", name.c_str(), !on_request);
        return kRPC_INVALID_PARAM;
    }

    if (false == m_service_map.insert({name, on_request}).second) {
        PEBBLE_PLOG_ERROR("the %s is existed", name.c_str());
...
void IRpc::GetResourceUsed(std::unordered_map<std::string, int64_t>* resource_info) {
    if (!resource_info) {
        return;
    }
...
int32_t IRpc::SendRequest(int64_t handle,
    ...
    if (buff_len != 0 && NULL == buff) {
        PEBBLE_PLOG_ERROR_N_EVERY_SECOND(1, "param invalid: buff = %p, buff_len = %u", buff, buff_len);
        return kRPC_INVALID_PARAM;
    }
    ...
    TimeoutCallback cb     = std::bind(&IRpc::OnTimeout, this, session->m_session_id);
...
int32_t IRpc::BroadcastRequest(const std::string& name,
    ...
    if (head_len < 0) {
        PEBBLE_PLOG_ERROR_N_EVERY_SECOND(1, "encode head failed(%d)", head_len);
...
int32_t IRpc::SendResponse(uint64_t session_id, int32_t ret,
    ...
    std::unordered_map< uint64_t, std::shared_ptr<RpcSession> >::iterator it =
        m_session_map.find(session_id);
    if (m_session_map.end() == it) {
        PEBBLE_PLOG_ERROR("session %lu not found", session_id);
...
int32_t IRpc::SendMessage(int64_t handle, const RpcHead& rpc_head,
    ...
    if (send_ret != 0) {
        PEBBLE_PLOG_ERROR_N_EVERY_SECOND(1, "send failed %d", send_ret);
    }
...
int32_t IRpc::ProcessRequestImp(int64_t handle, const RpcHead& rpc_head,
    ...
    std::unordered_map<std::string, OnRpcRequest>::iterator it =
        m_service_map.find(rpc_head.m_function_name);
    if (m_service_map.end() == it) {
        PEBBLE_PLOG_ERROR_N_EVERY_SECOND(1, "%s's request proc func not found", rpc_head.m_function_name.c_str());
    ...
    if (kRPC_ONEWAY == rpc_head.m_message_type) {
        std::function<int32_t(int32_t, const uint8_t*, uint32_t)> rsp; // NOLINT
    ...
    std::function<int32_t(int32_t, const uint8_t*, uint32_t)> rsp = std::bind( // NOLINT
...
int32_t IRpc::ProcessResponse(const RpcHead& rpc_head,
    ...
    if (m_session_map.end() == it) {
        PEBBLE_PLOG_ERROR_N_EVERY_SECOND(1, "session(%lu) not found, function_name(%s)",
    ...
    if (kRPC_EXCEPTION == rpc_head.m_message_type) {
        ...
        if (len < 0) {
            PEBBLE_PLOG_ERROR_N_EVERY_SECOND(1, "ExceptionDecode failed(%d), exception data len = %u", len, buff_len);
...
int32_t IRpc::ResponseException(int64_t handle, int32_t ret, const RpcHead& rpc_head,
    ...
    if (buff_len < sizeof(m_rpc_exception_buff)) {
        ...
        PEBBLE_PLOG_ERROR_N_EVERY_SECOND(1, "exception msg too long %u", buff_len);
    }

    int32_t result = ExceptionEncode(exception, m_rpc_exception_buff, sizeof(m_rpc_exception_buff));
    if (result < 0) {
        PEBBLE_PLOG_ERROR_N_EVERY_SECOND(1, "ExceptionEncode failed, ret = %d, exception data len = %u", result, buff_len);
        result = 0;
    }
