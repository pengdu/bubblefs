...
#ifndef BUBBLEFS_UTILS_DMLC_DATA_H_
#define BUBBLEFS_UTILS_DMLC_DATA_H_
  ...
  inline V SDot(const V *weight, size_t size) const {
    ...
    if (value == NULL) {
      for (size_t i = 0; i < length; ++i) {
        PANIC_ENFORCE(index[i] < size, "feature index exceed bound");
        sum += weight[index[i]];
      }
    } else {
      for (size_t i = 0; i < length; ++i) {
...
struct RowBlock {
  ...
  inline RowBlock Slice(size_t begin, size_t end) const {
    PANIC_ENFORCE(begin <= end && end <= size, "range invalid");
    RowBlock ret;
    ret.size = end - begin;
...
RowBlock<IndexType>::operator[](size_t rowid) const {
  PANIC_ENFORCE(rowid < size, "rowid should < size");
  Row<IndexType> inst;
...
#endif  // BUBBLEFS_UTILS_DMLC_DATA_H_