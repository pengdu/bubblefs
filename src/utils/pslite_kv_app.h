...
#ifndef BUBBLEFS_UTILS_PSLITE_KV_APP_H_
#define BUBBLEFS_UTILS_PSLITE_KV_APP_H_
...
#include "utils/pslite_simple_app.h"
...
class KVServer : public SimpleApp {
 public:
  ...
  void set_request_handle(const ReqHandle& request_handle) {
    PANIC_ENFORCE(request_handle, "invalid request handle");
...
struct KVServerDefaultHandle {
  void operator()(
    ...
    KVPairs<Val> res;
    if (req_meta.push) {
      PANIC_ENFORCE_EQ(n, req_data.vals.size());
    } else {
...
void KVServer<Val>::Process(const Message& msg) {
  ...
  int n = msg.data.size();
  if (n) {
    PANIC_ENFORCE_GE(n, 2);
    data.keys = msg.data[0];
    data.vals = msg.data[1];
    if (n > 2) {
      PANIC_ENFORCE_EQ(n, 3);
      data.lens = msg.data[2];
      PANIC_ENFORCE_EQ(data.lens.size(), data.keys.size());
    }
  }
  PANIC_ENFORCE(request_handle_, "request_handle_ is invalid");
...
void KVWorker<Val>::DefaultSlicer(
  ...
  for (size_t i = 0; i < n; ++i) {
    if (i == 0) {
      ...
      PANIC_ENFORCE_EQ(ranges[i-1].end(), ranges[i].begin());
    }
  ...
  PANIC_ENFORCE_EQ(pos[n], send.keys.size());
  ...
  if (send.lens.empty()) {
    ...
    PANIC_ENFORCE_EQ(k * send.keys.size(), send.vals.size());
  } else {
    PANIC_ENFORCE_EQ(send.keys.size(), send.lens.size());
  }
...
void KVWorker<Val>::Process(const Message& msg) {
  ...
  if (!msg.meta.push && msg.data.size()) {
    PANIC_ENFORCE_GE(msg.data.size(), (size_t)2);
    KVPairs<Val> kvs;
    kvs.keys = msg.data[0];
    kvs.vals = msg.data[1];
...
void KVWorker<Val>::RunCallback(int timestamp) {
  ...
  if (it != callbacks_.end()) {
    ...
    PANIC_ENFORCE(it->second, "it->second is invalid");
    it->second();
...
int KVWorker<Val>::Pull_(
  ...
  AddCallback(ts, [this, ts, keys, vals, lens, cb]() mutable {
      ...
      for (const auto& s : kvs) {
        ...
        PANIC_ENFORCE_EQ(range.size(), s.keys.size()); // "unmatched keys size from one server";
        if (lens) PANIC_ENFORCE_EQ(s.lens.size(), s.keys.size());
        total_key += s.keys.size();
        total_val += s.vals.size();
      }
      PANIC_ENFORCE_EQ(total_key, keys.size()); // "lost some servers?";
      ...
      PANIC_ENFORCE_NE(vals, NULL);
      if (vals->empty()) {
        vals->resize(total_val);
      } else {
        PANIC_ENFORCE_EQ(vals->size(), total_val);
      }
      ...
      if (lens) {
        if (lens->empty()) {
          ...
          PANIC_ENFORCE_EQ(lens->size(), keys.size());
        }
        p_lens = lens->data();
      }
      for (const auto& s : kvs) {
...
#endif  // BUBBLEFS_UTILS_PSLITE_KV_APP_H_