...
// dmlc-core/include/dmlc/lua.h

#ifndef BUBBLEFS_UTILS_DMLC_LUA_H_
#define BUBBLEFS_UTILS_DMLC_LUA_H_
...
#include "platform/dmlc_thread_local.h"
...
struct NumberHandler {
  static inline T Get(lua_State* L, int index, LuaState* s) {
    PANIC_ENFORCE_EQ(lua_type(L, index), LUA_TNUMBER);
...
struct MapHandler {
  ...
  static inline ContainerType Get(lua_State* L, int index, LuaState* s) {
    ContainerType ret;
    PANIC(lua_istable(L, index)),
          "Expected a table but get %c",
...
struct Handler<std::string> {
  static inline std::string Get(lua_State* L, int index, LuaState* s) {
    PANIC_ENFORCE_EQ(lua_type(L, index), LUA_TSTRING);
...
struct Handler<std::vector<T> > {
  static inline std::vector<T> Get(lua_State* L, int index, LuaState* s) {
    ...
    while (lua_next(L, tid)) {
      PANIC_ENFORCE_EQ(Handler<size_t>::Get(L, -2, s), ret.size() + 1); // "Target table is not an array"
...
inline LuaState::LuaState() {
  L_ = luaL_newstate();
  PANIC_ENFORCE(L_ != nullptr, "Failed to create new lua state");
  luaL_openlibs(L_);
}
...
inline void LuaState::PRun_(F f) {
  if (option_ != kLocking) {
    ...
    if (option_ == kThreadLocal) {
      PANIC_ENFORCE(ThreadLocalState() == this,
                    "Invoke lua from a different thread in ThreadLocal mode.");
    }
    f(L_);
    PANIC_ENFORCE_EQ(reset.top, lua_gettop(L_));
  } else {
...
inline LuaRef LuaState::Eval(const char* lua_code) {
  ...
  this->PRun_([this, lua_code, &ret](lua_State* L) {
      ...
      PANIC_ENFORCE(lua_pcall(L, 0, 1, 0) == 0,
                    "Lua call error: %d", lua_code);
...
inline T LuaRef::Get() const {
  PANIC_ENFORCE(state_ != nullptr, "Get:: LuaRef is nil");
  T ret;
...
inline LuaRef LuaRef::operator()(Args&& ...args) const {
  PANIC_ENFORCE(state_ != nullptr, "LuaRef is nil");
  ...
  state_->PRun_([this, nargs, &targ, &ret](lua_State* L) {
      ...
      PANIC_ENFORCE(lua_isfunction(L, -1),
                    "Expect to invoke a function but type=\'%c\'",
...
inline LuaRef& LuaRef::SetField(const std::string& key, const T& value) {  // NOLINT(*)
  ...
  state_->PRun_([this, &key, &value](lua_State* L) {
      lua_rawgeti(L, LUA_REGISTRYINDEX, this->ref_);
      PANIC_ENFORCE(lua_istable(L, -1),
                    "Expect a table but type=\'%c\'"
                    lua_typename(L, lua_type(L, -1)));
      lua_stack::Handler<T>::Push(L, value);
...
#endif  // BUBBLEFS_UTILS_DMLC_LUA_H_