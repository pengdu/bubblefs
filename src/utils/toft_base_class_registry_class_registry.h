...
// toft/base/class_registry/class_registry.h

#ifndef BUBBLEFS_UTILS_TOFT_BASE_CLASS_REGISTRY_CLASS_REGISTRY_H_
#define BUBBLEFS_UTILS_TOFT_BASE_CLASS_REGISTRY_CLASS_REGISTRY_H_
...
#define MYTOFT_CLASS_REGISTRY_DEFINE(registry_name, base_class_name) \
    struct registry_name##RegistryTag: \
        public ::bubblefs::mytoft::ClassRegistryTagBase<base_class_name> {};

#define MYTOFT_CLASS_REGISTRY_DEFINE_SINGLETON(registry_name, base_class_name) \
    struct registry_name##RegistryTag: \
        public ::bubblefs::mytoft::ClassRegistrySingletonTagBase<base_class_name> {};
...
//
#define MYTOFT_CLASS_REGISTRY_REGISTER_CLASS(registry_name, \
                                             base_class_name, \
                                             entry_name_as_string, \
                                             class_name) \
    static ::bubblefs::mytoft::ClassRegisterer<registry_name##RegistryTag> \
        MYTOFT_PP_JOIN(g_object_creator_registry_##class_name, __LINE__)( \
            entry_name_as_string, \
            &::bubblefs::mytoft::ClassRegistry_NewObject<base_class_name, class_name>)

#define MYTOFT_CLASS_REGISTRY_REGISTER_CLASS_SINGLETON(registry_name, \
                                                       base_class_name, \
                                                       entry_name_as_string, \
                                                       class_name) \
    static ::bubblefs::mytoft::ClassRegisterer<registry_name##RegistryTag> \
        MYTOFT_PP_JOIN(g_object_creator_registry_##class_name, __LINE__)( \
            entry_name_as_string, \
            &::bubblefs::mytoft::ClassRegistry_GetSingleton<class_name, registry_name##RegistryTag>)

// Create object from registry by name.
// Namespace prefix is required for registry_name if it is defined in different
// namespace
#define MYTOFT_CLASS_REGISTRY_CREATE_OBJECT(registry_name, entry_name_as_string) \
    ::bubblefs::mytoft::ClassRegistryInstance<registry_name##RegistryTag>().CreateObject(entry_name_as_string)

// Get object singleton from registry by name.
#define MYTOFT_CLASS_REGISTRY_GET_SINGLETON(registry_name, entry_name_as_string) \
    ::bubblefs::mytoft::ClassRegistryInstance<registry_name##RegistryTag>().GetSingleton(entry_name_as_string)

// Obtain the number of classes registered to the registry.
#define MYTOFT_CLASS_REGISTRY_CLASS_COUNT(registry_name) \
    ::bubblefs::mytoft::ClassRegistryInstance<registry_name##RegistryTag>().ClassCount()

// Obtain class name by index for the registry.
#define MYTOFT_CLASS_REGISTRY_CLASS_NAME(registry_name, i) \
    ::bubblefs::mytoft::ClassRegistryInstance<registry_name##RegistryTag>().ClassName(i)

#endif // BUBBLEFS_UTILS_TOFT_BASE_CLASS_REGISTRY_CLASS_REGISTRY_H_