...
// apollo/modules/control/filters/mean_filter.cc

#include "utils/apollo_mean_filter.h"
...
MF::MeanFilter(const uint8 window_size) : window_size_(window_size) {
  PANIC_ENFORCE_GT(window_size_, 0);
  PANIC_ENFORCE_LE(window_size_, kMaxWindowSize);
  initialized_ = true;
}
...
double MF::Update(const double measurement) {
  PANIC_ENFORCE(initialized_, "not initialized_");
  PANIC_ENFORCE_LE(values_.size(), window_size_);
  PANIC_ENFORCE_LE(min_candidates_.size(), window_size_);
  PANIC_ENFORCE_LE(max_candidates_.size(), window_size_);
  ++time_;
...
bool MF::ShouldPopOldestCandidate(const uint8 old_time) const {
  if (old_time < window_size_) {
    PANIC_ENFORCE_LE(time_, old_time + window_size_);
    return old_time + window_size_ == time_;
  } else if (time_ < window_size_) {
    PANIC_ENFORCE_GE(old_time, time_ + window_size_);
...
void MF::RemoveEarliest() {
  PANIC_ENFORCE_EQ(values_.size(), window_size_);
