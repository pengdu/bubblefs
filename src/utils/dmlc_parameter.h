...
#ifndef BUBBLEFS_UTILS_DMLC_PARAMETER_H_
#define BUBBLEFS_UTILS_DMLC_PARAMETER_H_
...
struct Parameter {
 public:
   ...
   */
  inline void Save(mydmlc::JSONWriter *writer) const {
   ...
   * \throw ParamError when something go wrong.
   */
  inline void Load(mydmlc::JSONReader *reader) {
...
#define DMLC_DECLARE_PARAMETER(PType)                                   \
  static ::bubblefs::mydmlc::parameter::ParamManager *__MANAGER__();                \
  inline void __DECLARE__(::bubblefs::mydmlc::parameter::ParamManagerSingleton<PType> *manager) \
...
#define DMLC_REGISTER_PARAMETER(PType)                                  \
  ::bubblefs::mydmlc::parameter::ParamManager *PType::__MANAGER__() {               \
    static ::bubblefs::mydmlc::parameter::ParamManagerSingleton<PType> inst(#PType); \
    return &inst.manager;                                               \
  }                                                                     \
  static DMLC_ATTRIBUTE_UNUSED ::bubblefs::mydmlc::parameter::ParamManager&         \
 ...
 public:
  ...
  inline void AddEntry(const std::string &key, FieldAccessEntry *e) {
    ...
    if (entry_map_.count(key) != 0) {
      PANIC("key has already been registered");
    }
    entry_.push_back(e);
    entry_map_[key] = e;
  }
  ...
  inline void AddAlias(const std::string& field, const std::string& alias) {
    if (entry_map_.count(field) == 0) {
      PANIC("key has not been registered");
    }
    if (entry_map_.count(alias) != 0) {
      PANIC("Alias has already been registered");
    }
...
template<typename DType>
class FieldEntry :
      public IfThenElseType<mydmlc::is_arithmetic<DType>::value,
...
class FieldEntry<int>
 ...
 protected:
  ...
  virtual void PrintValue(std::ostream &os, int value) const {  // NOLINT(*)
    if (is_enum_) {
      PANIC_ENFORCE_NE(enum_back_map_.count(value), 0U); // "Value not found in enum declared";
...
class FieldEntry<optional<int> >
 ...
 public:
  ...
  inline FieldEntry<optional<int> > &add_enum(const std::string &key, int value) {
    PANIC_ENFORCE_NE(key, "None"); // "None is reserved for empty optional<int>";
    if ((enum_map_.size() != 0 && enum_map_.count(key) != 0) || \
        enum_back_map_.count(value) != 0) {
      std::ostringstream os;
      os << "Enum " << "(" << key << ": " << value << " exisit!" << ")\n";
  ...
  virtual void PrintValue(std::ostream &os, optional<int> value) const {  // NOLINT(*)
    if (is_enum_) {
      if (!value) {
        ...
        PANIC_ENFORCE_NE(enum_back_map_.count(value.value()), 0U); // "Value not found in enum declared";
...
#endif  // BUBBLEFS_UTILS_DMLC_PARAMETER_H_