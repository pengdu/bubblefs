...
//ambry/ambry-utils/src/main/java/com.github.ambry.utils/BitSet.java

#ifndef BUBBLEFS_UTILS_AMBRY_BITSET_H_
#define BUBBLEFS_UTILS_AMBRY_BITSET_H_
...
class OpenBitSet {
   ...
   */
 private:
  int wlen; // number of words (elements) used in the array
  int pageCount;
 ...
 public: 
  OpenBitSet(long numBits) {
    ...
    if (lastPageSize != 0) {
      bits[pageCount - 1] = new long[lastPageSize];
    }
  }
  ...
  void clear(int startIndex, int endIndex) {
    ...
    long startmask = (unsigned int)-1L << startIndex;
    long endmask = (unsigned int)-1L >> -endIndex;  // 64-(endIndex&0x3f) is the same as -endIndex due to wrap
    ...
    int middle = std::min(wlen, endWord);
    if (startWord / PAGE_SIZE == middle / PAGE_SIZE) {
      long *p = bits[startWord / PAGE_SIZE];
      std::fill(p + (startWord + 1) % PAGE_SIZE, p + middle % PAGE_SIZE, 0L);
    } else {
  ...
  void intersect(OpenBitSet &other) {
    int newLen = std::min(this->wlen, other.wlen);
  ...
  static long bits2words(long numBits) {
    return (((unsigned long)(numBits - 1) >> 6) + 1);
  }

  /** returns true if both sets have the same bits set */
  bool equals(OpenBitSet *b) {
    if (nullptr == b)
      return false;
    if (this == b) {
      return true;
...
#endif // BUBBLEFS_UTILS_AMBRY_BITSET_H_