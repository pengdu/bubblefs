...
#include "utils/bitcoin_dbwrapper.h"
#include <stdint.h>
#include <algorithm>
...
CDBWrapper::CDBWrapper(const std::string& path, size_t nCacheSize, bool fMemory, bool fWipe, bool obfuscate)
{
    ...
    if (!key_exists && obfuscate && IsEmpty()) {
        ...
        printf("Wrote new obfuscate key for %s: %s\n", path.c_str(), HexStr(obfuscate_key).c_str());
    }

    printf("Using obfuscation key for %s: %s\n", path.c_str(), HexStr(obfuscate_key).c_str());
}
