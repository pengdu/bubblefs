...
// Teaf/trunk/teaf(isgw)/comm/plat_db_conn.h

#ifndef BUBBLEFS_UTILS_TEAF_PLAT_DB_CONN_H_
#define BUBBLEFS_UTILS_TEAF_PLAT_DB_CONN_H_
...
public:
    ...
    int exec_multi_query(const char* sql, std::vector<MYSQL_RES*> & result_set_list);
    // INSERT DELETE UPDATE
    int exec_update(const char* sql, int& last_insert_id, int& affected_rows);
    // TRANS 
    int exec_trans(const std::vector<std::string>& sqls, int& last_insert_id, int& affected_rows);
...
#endif // BUBBLEFS_UTILS_TEAF_PLAT_DB_CONN_H_
