...
// ps-lite/include/ps/sarray.h

#ifndef BUBBLEFS_UTILS_PSLITE_SARRAY_H_
#define BUBBLEFS_UTILS_PSLITE_SARRAY_H_
...
class SArray {
 public:
  ...
  template <typename W> void operator=(const SArray<W>& arr) {
    ...
    PANIC_ENFORCE(size_ * sizeof(V) == arr.size() * sizeof(W), "cannot be divided");
  ...
  inline V back() const { PANIC_ENFORCE(!empty(), "empty"); return data()[size_-1]; }
  inline V front() const { PANIC_ENFORCE(!empty(), "empty"); return data()[0]; }
  ...
  SArray<V> segment(size_t begin, size_t end) const {
    PANIC_ENFORCE_GE(end, begin); PANIC_ENFORCE_LE(end, size());
    SArray<V> ret;
...
#endif  // BUBBLEFS_UTILS_PSLITE_SARRAY_H_