...
// apollo/modules/common/math/box2d.cc

#include "utils/apollo_box2d.h"
...
Box2d::Box2d(const Vec2d &center, const double heading, const double length,
  ...
  PANIC_ENFORCE_GT(length_, -kMathEpsilon);
  PANIC_ENFORCE_GT(width_, -kMathEpsilon);
}
...
double Box2d::DistanceTo(const LineSegment2d &line_segment) const {
  ...
  PANIC("unimplemented state: %d %d %d %d", gx1, gy1, gx2, gy2);
  return 0.0;
}
...
std::string Box2d::DebugString() const {
  ...
  oss << "box2d ( center = " <<center_.DebugString()
