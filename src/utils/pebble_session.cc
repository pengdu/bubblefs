...
// Pebble/src/framework/session.cpp

#include "platform/pebble_timer.h"
#include "utils/pebble_session.h"
...
int32_t SessionMgr::AddSession(int64_t session_id, Session* session, uint32_t timeout_ms) {
    if (NULL == session || 0 == timeout_ms) {
        _PEBBLE_LOG_LAST_ERROR("invalid param: session = %p, timeout_ms = %u", session, timeout_ms);
        return kSESSION_INVALID_PARAM;
    }

    std::pair<std::unordered_map<int64_t, SessionInfo>::iterator, bool> ret =
        m_sessions.insert({session_id, SessionInfo()});
    if (false == ret.second) {
        _PEBBLE_LOG_LAST_ERROR("the session %ld is existed", session_id);
        return kSESSION_ALREADY_EXISTED;
    }

    TimeoutCallback cb = std::bind(&SessionMgr::OnTimeout, this, session_id, session);
    int64_t timerid = m_timer->StartTimer(timeout_ms, cb);
    if (timerid < 0) {
        m_sessions.erase(ret.first);
        _PEBBLE_LOG_LAST_ERROR("start timer failed(%s)", m_timer->GetLastError());
...
Session* SessionMgr::GetSession(int64_t session_id) {
    std::unordered_map<int64_t, SessionInfo>::iterator it = m_sessions.find(session_id);
    if (m_sessions.end() == it) {
        _PEBBLE_LOG_LAST_ERROR("the session %ld not exist", session_id);
        return NULL;
    }
...
int32_t SessionMgr::RestartTimer(int64_t session_id, uint32_t new_timeout_ms) {
    ...
    TimeoutCallback cb = std::bind(&SessionMgr::OnTimeout, this, session_id, it->second.m_session);
