...
#include "utils/caffe2_redis_store_handler.h"
...
RedisStoreHandler::RedisStoreHandler(
  ...
  PANIC_ENFORCE_NE(redis_, (redisContext*)nullptr);
  PANIC_ENFORCE_EQ(redis_->err, 0);
}
...
void RedisStoreHandler::set(const std::string& name, const std::string& data) {
  ...
  void* ptr = redisCommand(
      ...
      data.c_str(),
      (size_t)data.size());
  PANIC_ENFORCE_NE(ptr, (void*)nullptr);
  redisReply* reply = static_cast<redisReply*>(ptr);
  PANIC_ENFORCE_EQ(reply->type, REDIS_REPLY_INTEGER);
  PANIC_ENFORCE_EQ(reply->integer, 1); // "Value at name was already set (perhaps you reused a run ID you have used before?)"
}
...
std::string RedisStoreHandler::get(const std::string& name) {
  ...
  PANIC_ENFORCE_NE(ptr, (void*)nullptr);
  redisReply* reply = static_cast<redisReply*>(ptr);
  PANIC_ENFORCE_EQ(reply->type, REDIS_REPLY_STRING);
...
void RedisStoreHandler::wait(
  ...
  while (!check(names)) {
    ...
    if (timeout != kNoTimeout && elapsed > timeout) {
      STORE_HANDLER_TIMEOUT("Wait timeout for name(s): ", JoinToString(" ", names));
    }
    /* sleep override */
