...
// pdlfs-common/src/fstypes_test.cc
...
TEST(KeyTest, KeyEncoding) {
  ...
  }
  {
    ...
    k3.SetName(Slice("Y"));
    ASSERT_EQ(k1.Encode().as_string(), Slice(zero, k1.size()).as_string());
    ASSERT_EQ(k2.prefix().as_string(), k3.prefix().as_string());
    ASSERT_NE(k2.Encode().as_string(), k3.Encode().as_string());
    Key k4(k3);
    Key k5 = k3;
    k4.SetName(Slice("X"));
    ASSERT_EQ(k4.Encode().as_string(), k2.Encode().as_string());
    k5.SetHash(k2.hash());
    ASSERT_EQ(k5.Encode().as_string(), k2.Encode().as_string());
  }
  {
    ...
    k2.SetOffset(32);
    k3.SetOffset(128);
    ASSERT_LT(k1.Encode().as_string(), k2.Encode().as_string());
    ASSERT_LT(k2.Encode().as_string(), k3.Encode().as_string());
  ...
  }
  {
    ...
    Key k2(k1.prefix());
    ASSERT_EQ(k1.prefix().as_string(), k2.prefix().as_string());
    Key k3(31, kDataBlockType);
    k3.SetType(kDataDesType);
    ASSERT_EQ(k1.prefix().as_string(), k3.prefix().as_string());
  }
}
