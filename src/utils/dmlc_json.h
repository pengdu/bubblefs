...
#ifndef BUBBLEFS_UTILS_DMLC_JSON_H_
#define BUBBLEFS_UTILS_DMLC_JSON_H_
...
#define MYDMLC_JSON_ENABLE_ANY_VAR_DEF(KeyName)                  \
  static ATTRIBUTE_UNUSED ::bubblefs::mydmlc::json::AnyJSONManager&   \
 ...
 */
#define MYDMLC_JSON_ENABLE_ANY(Type, KeyName)                             \
  MYDMLC_STR_CONCAT(MYDMLC_JSON_ENABLE_ANY_VAR_DEF(KeyName), __COUNTER__) = \
    ::bubblefs::mydmlc::json::AnyJSONManager::Global()->EnableType<Type>(#KeyName) \
...
struct ArrayHandler {
  inline static void Write(JSONWriter *writer, const ContainerType &array) {
    ...
    writer->BeginArray(array.size() > 10 || !mydmlc::is_pod<ElemType>::value);
...
struct Handler<std::pair<K, V> > {
  ...
  inline static void Read(JSONReader *reader, std::pair<K, V> *kv) {
    reader->BeginArray();
    PANIC_ENFORCE(reader->NextArrayItem(),
...
struct Handler {
  inline static void Write(JSONWriter *writer, const T &data) {
    typedef typename mydmlc::IfThenElseType<mydmlc::is_arithmetic<T>::value,
                                          NumericHandler<T>,
...
class AnyJSONManager {
 public:
  ...
  inline AnyJSONManager& EnableType(const std::string& type_name) {  // NOLINT(*)
    ...
    if (type_name_.count(tp) != 0) {
      PANIC_ENFORCE(type_name_.at(tp) == type_name,
                    "Type has already been registered as another typename %s", type_name_.at(tp).c_str());
      return *this;
    }
    PANIC_ENFORCE(type_map_.count(type_name) == 0,
                  "Type name %s already registered in registry", type_name.c_str());
    Entry e;
    e.read = ReadAny<T>;
    e.write = WriteAny<T>;
 ...
 private:
  ...
  inline static void WriteAny(JSONWriter *writer, const any &data) {
    writer->Write(mydmlc::get<T>(data));
  }
  template<typename T>
...
struct Handler<any> {
  ...
  inline static void Read(JSONReader *reader, any *data) {
    ...
    reader->BeginArray();
    PANIC_ENFORCE(reader->NextArrayItem(), "invalid any json format");
    ...
    PANIC_ENFORCE(it != tmap.end() && it->first == type_name,
                  "Typename %s has not been registered via MYDMLC_JSON_ENABLE_ANY", type_name.c_str());
  ...
  while (true) {
    ...
    if (ch == '\\') {
      ...
      switch (sch) {
        ...
        case 't': os << "\t"; break;
        case '\"': os << "\""; break;
        default: PANIC("unknown string escape \\%c", sch);
      }
    } else {
      if (ch == '\"') break;
    ...
    if (ch == EOF || ch == '\r' || ch == '\n') {
      PANIC("Expect \'\"\' but reach end of line ");
    }
  }
  *out_str = os.str();
}
...
inline void JSONReader::ReadNumber(ValueType *out_value) {
  *is_ >> *out_value;
  PANIC_ENFORCE(!is_->fail(), "Expect number");
}

inline void JSONReader::BeginObject() {
  int ch = NextNonSpace();
  PANIC_ENFORCE_EQ(ch, '{'); // "Expect \'{\' but get " << static_cast<char>(ch)
...
inline bool JSONReader::NextObjectItem(std::string *out_key) {
  ...
  if (scope_counter_.back() != 0) {
    ...
    if (ch == EOF) {
      ...
      PANIC_ENFORCE_EQ(ch, ','); // "JSON object expect \'}\' or \',\' \'" << static_cast<char>(ch)
    }
  } else {
...
inline bool JSONReader::NextArrayItem() {
  ...
  if (scope_counter_.back() != 0) {
    ...
    if (ch == EOF) {
      ...
      PANIC_ENFORCE_EQ(ch, ','); // "JSON array expect \']\' or \',\'. Get \'" << static_cast<char>(ch) << "\' instead";
    }
  } else {
    int ch = PeekNextNonSpace();
    if (ch == ']') {
...
inline void JSONWriter::EndArray() {
  PANIC_ENFORCE_NE(scope_multi_line_.size(), 0U);
  PANIC_ENFORCE_NE(scope_counter_.size(), 0U);
...
inline void JSONWriter::Write(const ValueType &value) {
  ...
  json::Handler<ValueType>::Write(this, value);
  PANIC_ENFORCE_EQ(nscope, scope_multi_line_.size()); // "Uneven scope, did you call EndArray/EndObject after each BeginObject/Array?";
}
...
inline void JSONObjectReadHelper::ReadAllFields(JSONReader *reader) {
  ...
  if (visited.size() != map_.size()) {
    for (std::map<std::string, Entry>::iterator
      ...
      PANIC_ENFORCE_NE(visited.count(it->first), 0U); // "JSONReader: Missing field \"" << it->first
    }
  }
}
...
DeclareFieldInternal(const std::string &key, T *addr, bool optional) {
  PANIC_ENFORCE_EQ(map_.count(key), 0U) // "Adding duplicate field " << key;
  Entry e;
...
#endif  // BUBBLEFS_UTILS_DMLC_JSON_H_