...
// apollo/modules/common/util/file.cc
...
std::string GetRosHome() {
  ...
  const std::string ros_root = std::getenv("ROS_ROOT");
  PANIC_ENFORCE(EndWith(ros_root, known_tail), "not /share/ros");
...
bool CopyFile(const std::string &from, const std::string &to) {
  ...
  if (!src) {
    PRINTF_ERROR("Source path doesn't exist: %s\n", from.c_str());
    return false;
  }

  std::ofstream dst(to, std::ios::binary);
  if (!dst) {
    PRINTF_ERROR("Target path is not writable: %s\n", to.c_str());
    return false;
  }
...
bool CopyDir(const std::string &from, const std::string &to) {
  ...
  if (directory == nullptr) {
    PRINTF_ERROR("Cannot open directory %s\n", from.c_str());
    return false;
  }
  if (!EnsureDirectory(to)) {
    PRINTF_ERROR("Cannot create target directory %s\n", to.c_str());
    return false;
  }
...
bool RemoveAllFiles(const std::string &directory_path) {
  ...
  while ((file = readdir(directory)) != nullptr) {
    ...
    if (unlink(file_path.c_str()) < 0) {
      PRINTF_ERROR("Fail to remove file %s : %s\n", file_path.c_str(), strerror(errno));
      closedir(directory);
      return false;
    }
  }
