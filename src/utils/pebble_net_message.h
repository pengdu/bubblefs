...
// Pebble/src/framework/net_message.h

#ifndef BUBBLEFS_UTILS_PEBBLE_NET_MESSAGE_H_
#define BUBBLEFS_UTILS_PEBBLE_NET_MESSAGE_H_
...
typedef std::function<int32_t(const uint8_t* head, uint32_t head_len)> GetMsgDataLen;
...
private:
    ...
    // 连接数据
    std::unordered_map<uint64_t, NetConnection*> m_connections;

    // udp <peer handle, local listen handle> map
    std::unordered_map<uint64_t, uint64_t> m_peer_handle_to_local;
};
...
#endif // BUBBLEFS_UTILS_PEBBLE_NET_MESSAGE_H_