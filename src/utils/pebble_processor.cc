...
// Pebble/src/framework/processor.cpp

#include "platform/pebble_logging.h"
#include "utils/pebble_processor.h"
...
int32_t IProcessor::SetSendFunction(const SendFunction& send, const SendVFunction& sendv) {
    if (!send || !sendv) {
        PEBBLE_PLOG_ERROR("param invalid: !send = %d, !sendv = %d", !send, !sendv);
...
int32_t IProcessor::SetBroadcastFunction(const BroadcastFunction& broadcast,
    ...
    if (!broadcast || !broadcastv) {
        PEBBLE_PLOG_ERROR("param invalid: !broadcast = %d, !broadcastv = %d", !broadcast, !broadcastv);
        return kPROCESSOR_INVALID_PARAM;
    }
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
// 避免全局变量构造、析构顺序问题
static std::unordered_map<int32_t, std::shared_ptr<ProcessorFactory> > * g_processor_factory_map = NULL;
struct ProcessorFactoryMapHolder {
    ...
    std::unordered_map<int32_t, std::shared_ptr<ProcessorFactory> > _processor_factory_map;
};

std::shared_ptr<ProcessorFactory> GetProcessorFactory(int32_t type) {
    std::shared_ptr<ProcessorFactory> null_factory;
    if (!g_processor_factory_map) {
        PEBBLE_PLOG_ERROR("processor factory map not inited.");
        return null_factory;
    }

    std::unordered_map<int32_t, std::shared_ptr<ProcessorFactory> >::iterator it =
        g_processor_factory_map->find(type);
    if (it != g_processor_factory_map->end()) {
        return it->second;
    }

    PEBBLE_PLOG_ERROR("processor type %d not registered.", type);
    return null_factory;
}

int32_t SetProcessorFactory(int32_t type, const std::shared_ptr<ProcessorFactory>& factory) {
    static ProcessorFactoryMapHolder processor_factory_map_holder;
    if (!factory) {
        PEBBLE_PLOG_ERROR("factory is null");
        return kPROCESSOR_INVALID_PARAM;
    }
    ...
    if (false == g_processor_factory_map->insert({type, factory}).second) {
        PEBBLE_PLOG_ERROR("processor type %d is already registered", type);
