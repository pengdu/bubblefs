...
#ifndef BUBBLEFS_UTILS_NNVM_TUPLE_H_
#define BUBBLEFS_UTILS_NNVM_TUPLE_H_
...
template<typename T>
struct hash<bubblefs::mynnvm::Tuple<T> > {
  /*! \brief hash a Tuple into unsigned int */
  size_t operator()(const bubblefs::mynnvm::Tuple<T>& val) const {
    std::hash<uint32_t> hash_uint;
    size_t res = hash_uint(val.ndim());
    for (uint32_t i = 0; i < val.ndim(); ++i) {
      res = bubblefs::mydmlc::HashCombine(res, val[i]);
    }
    return res;
  }
};
...
template<>
struct hash<bubblefs::mynnvm::TShape> {
  /*! \brief hash a TShape into unsigned int */
  size_t operator()(const bubblefs::mynnvm::TShape& val) const {
    std::hash<uint32_t> hash_uint;
    size_t res = hash_uint(val.ndim());
    for (uint32_t i = 0; i < val.ndim(); ++i) {
      res = bubblefs::mydmlc::HashCombine(res, val[i]);
...
#if !defined(_MSC_VER)
template<typename T>
struct type_name_helper<bubblefs::mynnvm::Tuple<T> > {
...
#endif  // BUBBLEFS_UTILS_NNVM_TUPLE_H_