...
// Pebble/src/framework/rpc.h

#ifndef BUBBLEFS_UTILS_PEBBLE_RPC_H_
#define BUBBLEFS_UTILS_PEBBLE_RPC_H_
...
/// @return 0 处理成功
/// @return 非0 处理失败
typedef std::function<int32_t(const uint8_t* buff, uint32_t buff_len, // NOLINT
            std::function<int32_t(int32_t ret, const uint8_t* buff, uint32_t buff_len)>& rsp)> OnRpcRequest; // NOLINT

/// @brief RPC响应消息处理函数原型，由用户实现，RPC负责回调
/// @param ret RPC调用结果，成功为0，其他非0 @see RpcErrorCode
/// @param buff 响应消息码流
/// @param buff_len 响应消息长度
typedef std::function<int32_t(int32_t ret, const uint8_t* buff, uint32_t buff_len)> OnRpcResponse;
...
public:
    ...
    virtual void GetResourceUsed(std::unordered_map<std::string, int64_t>* resource_info);
...
private:
    std::unordered_map<std::string, OnRpcRequest> m_service_map;
    ...
    uint64_t m_session_id;
    std::unordered_map< uint64_t, std::shared_ptr<RpcSession> > m_session_map;
...
#endif // BUBBLEFS_UTILS_PEBBLE_RPC_H_