...
// pdlfs-common/src/env.cc

#include "platform/pdlfs_env.h"
#include <stdio.h>
#include "platform/pdlfs_env_files.h"
#include "platform/pdlfs_env_lazy.h"
...
#include "platform/pdlfs_posix_logger.h"
#endif
...
Env* Env::Open(const char* name, const char* conf, bool* is_system) {
  *is_system = false;
  ...
  Verbose(__PDLFS_LOG_ARGS__, 1, "env.name -> %s", env_name_str);
  Verbose(__PDLFS_LOG_ARGS__, 1, "env.conf -> %s", env_conf_str);
#endif
// RADOS
#if defined(PDLFS_RADOS)
  if (env_name == "rados") {
...
#endif
  if (env_name.empty()) {
    Warn(__PDLFS_LOG_ARGS__, "Open env without specifying a name...");
  }
