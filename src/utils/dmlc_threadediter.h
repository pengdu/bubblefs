...
#ifndef BUBBLES_UTILS_DMLC_THREADEDITER_H_
#define BUBBLES_UTILS_DMLC_THREADEDITER_H_
...
class ThreadedIter : public DataIter<DType> {
 public:
  ...
  virtual const DType &Value(void) const {
    PANIC_ENFORCE(out_data_ != NULL, "Calling Value at beginning or end?");
    return *out_data_;
  }
  ...
  virtual void BeforeFirst(void) {
    ...
    PANIC_ENFORCE(!producer_sig_processed_, "");
 ...
 private:
  ...
  inline static void NotImplemented(void) {
    PANIC("BeforeFirst is not supported");
  }
...
Init(Producer *producer, bool pass_ownership) {
  PANIC_ENFORCE(producer_owned_ == NULL, "can only call Init once");
    ...
    while (true) {
      DType *cell = NULL;
      {
        ...
        if (producer_sig_ == kProduce) {
          ...
          // destroy the thread
          PANIC_ENFORCE(producer_sig_ == kDestroy, "");
          producer_sig_processed_ = true;
          produce_end_ = true;
          consumer_cond_.notify_all();
          return;
      ...
      PANIC_ENFORCE(cell != NULL || produce_end_, "");
      bool notify;
      {
        // lockscope
        std::lock_guard<std::mutex> lock(mutex_);
...
Next(DType **out_dptr) {
  ...
  std::unique_lock<std::mutex> lock(mutex_);
  PANIC_ENFORCE(producer_sig_ == kProduce, "Make sure you call BeforeFirst not inconcurrent with Next!");
  ++nwait_consumer_;
  consumer_cond_.wait(lock, [this]() {
...
#endif  // BUBBLES_UTILS_DMLC_THREADEDITER_H_