...
// Pebble/src/framework/channel_mgr.cpp

#include "utils/pebble_channel_mgr.h"
...
int32_t ChannelMgr::JoinChannel(const std::string& name, Subscriber subscriber) {
    std::unordered_map<std::string, SubscriberList>::iterator it = m_channels.find(name);
...
int32_t ChannelMgr::QuitChannel(Subscriber subscriber) {
    int32_t ret = 0;
    int32_t num = 0;
    std::unordered_map<std::string, SubscriberList>::iterator it = m_channels.begin();
