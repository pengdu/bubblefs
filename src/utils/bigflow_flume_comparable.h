...
// bigflow/flume/util/comparable.h

#ifndef  BUBBLEFS_UTILS_BIGFLOW_FLUME_COMPARABLE_H_
#define  BUBBLEFS_UTILS_BIGFLOW_FLUME_COMPARABLE_H_
...
inline void AppendOrdered(T key, std::string* s) {
    ...
    MYTOFT_STATIC_ASSERT(sizeof(typename T::unsupported_type_for_AppendOrder) == INT_MAX);
}
...
inline void AppendReverseOrdered(T key, std::string* s) {
    // refuse to specialize unsupport type
    MYTOFT_STATIC_ASSERT(sizeof(typename T::unsupported_type_for_AppendReverseOrder) == INT_MAX);
}
...
inline void AppendOrdered(uint16_t key, std::string* s) {
    key = mytoft::ByteOrder::ToNet<uint16_t>(key);
    s->append(reinterpret_cast<char*>(&key), sizeof(key));
}

template <>
inline void AppendOrdered(uint32_t key, std::string* s) {
    key = mytoft::ByteOrder::ToNet<uint32_t>(key);
    s->append(reinterpret_cast<char*>(&key), sizeof(key));
}

template <>
inline void AppendOrdered(uint64_t key, std::string* s) {
    key = mytoft::ByteOrder::ToNet<uint64_t>(key);
    s->append(reinterpret_cast<char*>(&key), sizeof(key));
}
...
#endif  // BUBBLEFS_UTILS_BIGFLOW_FLUME_COMPARABLE_H_