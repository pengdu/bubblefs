...
#ifndef BUBBLEFS_UTILS_PADDLE_THREAD_H_
#define BUBBLEFS_UTILS_PADDLE_THREAD_H_
...
public:
  ...
  void exec(JobFunc jobFunc, JobFunc ownerFunc = nullptr) {
    if (checkOwner_) {
      PANIC_ENFORCE_EQ(ownerThreadId_, getTID()); // "this sync thread pool should be used in one thread";
    }

    PANIC_ENFORCE_EQ(jobFunc_, nullptr);
    jobFunc_ = jobFunc;
...
public:
   ...
   */
  void stop() {
    PANIC_ENFORCE(!jobAdding_, "stopAddJob() should be called before stop()");
    for (auto& worker : workers_) {
      if (worker) {
        worker->join();
        worker = nullptr;
   ...
   */
  void addJob(JobFunc func) {
    PANIC_ENFORCE(jobAdding_, "addJob() can not be called after stopAddJob()");
    jobs_.enqueue(func);
  }
...
public:
  ...
  AsyncThreadPool() { PANIC("Not implemented"); }
  ...
  auto addJob(F&& f, Args&&... args)
    ...
    PANIC_ENFORCE(!stopping_, "AsyncThreadPool is closed");
...
#endif // BUBBLEFS_UTILS_PADDLE_THREAD_H_