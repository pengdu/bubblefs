...
// Pebble/src/framework/dr/protocol/rapidjson_protocol.cpp

#define RAPIDJSON_HAS_STDSTRING 1

#include "utils/pebble_dr_rapidjson_protocol.h"
#include "utils/pebble_dr_transport_exception.h"
#include <limits>
#include <iostream>
#include <sstream>
#include "utils/pebble_dr_base64_utils.h"
...
using namespace mypebble::dr::transport;  
...
TRAPIDJSONProtocol::TRAPIDJSONProtocol(std::shared_ptr<TTransport> ptrans) :
...
uint32_t TRAPIDJSONProtocol::readMessageBegin(std::string& name,
  ...
  contexts_stack_.push(std::shared_ptr<TraverseContext>(new ArrayTraverseContext(itr, root.End())));
...
uint32_t TRAPIDJSONProtocol::readStructBegin(std::string& name) {
  ...
  contexts_stack_.push(std::shared_ptr<TraverseContext>(new ObjectTraverseContext(obj)));
  return readed;
}
...
uint32_t TRAPIDJSONProtocol::readMapBegin(TType& keyType,
  ...
  valType = T_NULL;
  size = map.Size() / 2;
  contexts_stack_.push(std::shared_ptr<TraverseContext>(new ObjectTraverseContext(map)));
  return result;
}
...
uint32_t TRAPIDJSONProtocol::readListBegin(TType& elemType,
  ...
  contexts_stack_.push(std::shared_ptr<TraverseContext>(new ArrayTraverseContext(list.Begin(), list.End())));
...
uint32_t TRAPIDJSONProtocol::skip(TType type) {
  if (type == T_NULL) {
    ...
    return dr::protocol::skip(*this, type);
  }
}
