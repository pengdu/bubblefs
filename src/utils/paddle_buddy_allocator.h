...
// Paddle/paddle/memory/detail/buddy_allocator.h
...
#include "platform/paddle_gpu_info.h"
#include "utils/paddle_memory_meta_cache.h"
#include "utils/paddle_memory_meta_data.h"
#include "utils/paddle_system_allocator.h"
...
// BuddyAllocator implements the buddy allocation algorithm.
// Please be aware that BuddyAllocator always allocate aligned memory, 
// aligned on 32-bytes, which can hold a BuddyAllocator::Block object:  
// Because BuddyAllocator has the meta-data of each block, 
// it can trace the used memory -- record the amount returned by Alloc freed in Free. 
// Instead, CPUAllocator and GPUAllocator doesn't know the size of freed memory block and cannot do the trace.
