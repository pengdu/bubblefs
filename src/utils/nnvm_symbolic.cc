...
#include "utils/nnvm_graph.h"
#include "utils/nnvm_symbolic.h"
#include "utils/nnvm_op_attr_types.h"
...
inline void UpdateNodeVersion(Node *n) {
  ...
  for (NodeEntry& e : n->inputs) {
    if (e.node->is_variable()) {
      e.version = mynnvm::get<VariableParam>(e.node->attrs.parsed).version;
    }
  }
  if (fmutate_inputs.count(n->op()) != 0) {
    for (uint32_t i : fmutate_inputs[n->op()](n->attrs)) {
      NodeEntry& e = n->inputs[i];
      PANIC_ENFORCE(e.node->is_variable(), "Mutation target can only be Variable");
...
inline void KeywordArgumentMismatch(const char *source,
  ...
  for (const auto& key : user_args) {
    if (keys.count(key) == 0) {
      PANIC("Keyword argument name not found.");
    }
  }
}
...
Symbol Symbol::operator[] (size_t index) const {
  ...
  PANIC_ENFORCE_LT(index, nreturn);
  if (nreturn == 1) {
    return *this;
  } else {
    Symbol s;
...
void Symbol::Compose(const array_view<const Symbol*>& args,
  ...
  // parameter check.
  for (size_t i = 0; i < args.size(); ++i) {
    PANIC_ENFORCE_EQ(args[i]->outputs.size(), 1U); // "Argument " << i << " is a tuple, single value is required";
  }
  for (const auto& kv : kwargs) {
    PANIC_ENFORCE_EQ(kv.second->outputs.size(), 1U); // "Keyword Argument " << kv.first << " is a tuple, single value is required";
  }
  // assign new name
  ...
  if (IsAtomic(outputs)) {
    ...
    if (n_req != kVarg) {
      ...
      PANIC_ENFORCE_LE(args.size(), n_req); // << "Incorrect number of arguments, requires " << n_req << ", provided " << args.size();
      for (size_t i = 0; i < args.size(); ++i) {
      ...
      if (args.size() != n_req) {
        ...
        if (arg_names.size() != n_req) {
          PANIC("Not enough argument to call operator"); // << outputs[0].node->op()->name;
        }
        size_t nmatched = 0;
        ...
        if (nmatched != kwargs.size()) {
          ...
          array_view<std::string> view(mydmlc::BeginPtr(arg_names) + args.size(),
                                       mydmlc::BeginPtr(arg_names) + arg_names.size());
          KeywordArgumentMismatch("Symbol.Compose", keys, view);
        }
      }
    } else {
      PANIC_ENFORCE_EQ(kwargs.size(), 0U); // "Variable length function do not accept kwargs";
    ...
    // general composition
    PANIC_ENFORCE_EQ(args.size(), 0U); // "General composition only support kwargs for now";
    size_t nmatched = 0;
    ...
    if (nmatched == kwargs.size() && arg_counter <= args.size()) {
      ...
      array_view<std::string> view(mydmlc::BeginPtr(arg_names) + arg_counter,
                                   mydmlc::BeginPtr(arg_names) + arg_names.size());
    ...
    for (size_t i = 0; i < outputs.size(); ++i) {
      if (outputs[i].node->is_variable()) {
        PANIC_ENFORCE_EQ(args.size(), 0); // "Variable composition only supports keyword arguments";
...
void Symbol::AddControlDeps(const Symbol& src) {
  PANIC_ENFORCE_EQ(outputs.size(), 1U); // "AddControlDeps only works for nongrouped symbol";
  Node* n = outputs[0].node.get();
...
Symbol Symbol::GetInternals() const {
  ...
  DFSVisit(this->outputs, [&ret](const NodePtr& node) {
      ...
      if (n->is_variable()) {
        ...
        VariableParam& param = mynnvm::get<VariableParam>(n->attrs.parsed);
...
void Symbol::SetAttrs(const std::vector<std::pair<std::string, std::string> >& attrs) {
  Node* node = outputs[0].node.get();
  for (const NodeEntry& e : outputs) {
    PANIC_ENFORCE(node == e.node.get(), "Symbol.SetAttrs only works for non-grouped symbol");
  }
