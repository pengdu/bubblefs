...
// caffe2/caffe2/core/operator_schema.h

#ifndef BUBBLEFS_UTILS_CAFFE2_OPERATOR_SCHEMA_H_
#define BUBBLEFS_UTILS_CAFFE2_OPERATOR_SCHEMA_H_
...
class OpSchema {
 public:
  ...
  OpSchema& EnforceInplace(std::set<std::pair<int, int>> inplace);
  ...
  typedef std::function<
      std::vector<TensorShape>(const OperatorDef&, const std::vector<TensorShape>&)>
  ...
  OpSchema& ScalarType(TensorProto::DataType dt);

  /**
   * @brief A function to allow one to infer the type and shape from the op
   * schema.
   */
  inline std::vector<TensorShape> InferTensor(
      const OperatorDef& def,
      const std::vector<TensorShape> input_type_shape) const {
   ...
   */
  typedef std::function<
      struct Cost(const OperatorDef&, const std::vector<TensorShape>&)>
...
#undef DECLARE_STANDARD_ARG
 ...
 private:
  ...
  TensorInferenceFunctionType tensor_inference_function_ =
      [](const OperatorDef& def, const std::vector<TensorShape>&) {
        std::vector<TensorShape> out;
        for (int i = 0; i < def.output.size(); i++) {
          TensorShape ts;
          ts.unknown_shape = true;
          out.push_back(ts);
        }
        return out;
      };
  ...
  DeviceInferenceFunctionType device_inference_function_ =
      [](const OperatorDef& def) {
        auto op_device =
            def.device_option ? def.device_option : DeviceOption();
        std::vector<DeviceOption> in_dev(def.input.size(), op_device);
        std::vector<DeviceOption> out_dev(def.output.size(), op_device);
...
// Helper function
inline std::vector<TIndex> GetDimsVector(const TensorShape& shape) {
...
#endif // BUBBLEFS_UTILS_CAFFE2_OPERATOR_SCHEMA_H_