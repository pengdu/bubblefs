...
// bigflow/flume/util/jni_environment.h

#ifndef BUBBLEFS_UTILS_BIGFLOW_FLUME_JNI_ENVIRONMENT_H_
#define BUBBLEFS_UTILS_BIGFLOW_FLUME_JNI_ENVIRONMENT_H_
...
public:
    ...
    static void check_and_describe(JNIEnv* env, JTYPE object) {
        if (reinterpret_cast<JTYPE>(NULL) == object) {
            ...
            PANIC("Error getting JVM object");
        }
    }
    ...
    static void check_and_describe(JNIEnv* env) {
        if (env->ExceptionCheck()) {
            env->ExceptionDescribe();
            env->ExceptionClear();
            PANIC("Erroring calling method, see errors below");
        }
    }
...
#endif  // BUBBLEFS_UTILS_BIGFLOW_FLUME_JNI_ENVIRONMENT_H_