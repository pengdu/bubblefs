...
#ifndef  BUBBLEFS_UTILS_BIGFLOW_FLUME_CACHED_ALLOCATOR_H_
#define  BUBBLEFS_UTILS_BIGFLOW_FLUME_CACHED_ALLOCATOR_H_

#include <cstdlib>
#include <memory>

#include "platform/base_error.h"
#include "utils/toft_base_singleton.h"

namespace bubblefs {
namespace mybdflume {

class MemoryPieceStore : public mytoft::SingletonBase<MemoryPieceStore> {
public:
    friend class mytoft::SingletonBase<MemoryPieceStore>;
    ...
    void Release(void* ptr, size_t sz) {
        ...
        assert(sz > 0U && sz <= static_cast<size_t>(kMaxCacheByte)
    ...
    MemoryPieceStore() : m_allocate_count(0), m_release_count(0), m_length(0), m_buffer(NULL) {
        ...
        PRINTF_INFO("total allocate cache size: %d\n", m_length);
...
#endif  // BUBBLEFS_UTILS_BIGFLOW_FLUME_CACHED_ALLOCATOR_H_