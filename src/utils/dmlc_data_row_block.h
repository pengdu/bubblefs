...
#ifndef BUBBLEFS_UTILS_DMLC_DATA_STRTONUM_H_
#define BUBBLEFS_UTILS_DMLC_DATA_STRTONUM_H_
  ...
  inline void Push(Row<I> row) {
    ...
    if (row.field != NULL) {
      for (size_t i = 0; i < row.length; ++i) {
        PANIC_ENFORCE_LE(row.field[i], std::numeric_limits<IndexType>::max()); // "field exceed numeric bound of current type";
    ...
    for (size_t i = 0; i < row.length; ++i) {
      PANIC_ENFORCE_LE(row.index[i], std::numeric_limits<IndexType>::max()); // "index exceed numeric bound of current type";
  ...
  inline void Push(RowBlock<I> batch) {
    ...
    if (batch.field != NULL) {
      ...
      for (size_t i = 0; i < ndata; ++i) {
        PANIC_ENFORCE_LE(batch.field[i], std::numeric_limits<IndexType>::max()); // "field  exceed numeric bound of current type";
    ...
    for (size_t i = 0; i < ndata; ++i) {
      PANIC_ENFORCE_LE(batch.index[i], std::numeric_limits<IndexType>::max()); // "index  exceed numeric bound of current type";
...
RowBlockContainer<IndexType>::GetBlock(void) const {
  // consistency check
  if (label.size()) {
    PANIC_ENFORCE_EQ(label.size() + 1, offset.size());
  }
  PANIC_ENFORCE_EQ(offset.back(), index.size());
  PANIC(offset.back() == value.size() || value.size() == 0, "value.size() is invalid");
...
RowBlockContainer<IndexType>::Load(Stream *fi) {
  ...
  PANIC(fi->Read(&label), "Bad RowBlock format");
  PANIC(fi->Read(&weight), "Bad RowBlock format");
  PANIC(fi->Read(&field), "Bad RowBlock format");
  PANIC(fi->Read(&index), "Bad RowBlock format");
  PANIC(fi->Read(&value), "Bad RowBlock format");
  PANIC(fi->Read(&max_field, sizeof(IndexType)), "Bad RowBlock format");
  PANIC(fi->Read(&max_index, sizeof(IndexType)), "Bad RowBlock format");
  return true;
}
...
#endif  // BUBBLEFS_UTILS_DMLC_DATA_ROW_BLOCK_H_