...
#ifndef BUBBLEFS_UTILS_APOLLO_ADAPTERS_ADAPTER_H_
#define BUBBLEFS_UTILS_APOLLO_ADAPTERS_ADAPTER_H_
...
#include "platform/apollo_time.h"
#include "utils/apollo_file.h"
...
class Adapter : public AdapterBase {
 public:
  ...
  Adapter(const std::string& adapter_name, const std::string& topic_name,
    ...
    if (HasSequenceNumber<D>()) {
      if (!common::util::EnsureDirectory(dump_path_)) {
        PANIC("Cannot enable dumping for '%s"
               "' adapter because the path %s"
               " cannot be created or is not a directory.",
               adapter_name.c_str(), dump_path_.c_str());
        enable_dump_ = false;
      } else if (!common::util::RemoveAllFiles(dump_path_)) {
        PANIC("Cannot enable dumping for '%s"
               "' adapter because the path %s"
  ...
  void FillHeader(const std::string& module_name, D* data) {
    ...
    double timestamp = common::time::Clock::NowInSeconds();
  ...
  bool DumpLatestMessage() override {
    ...
    PRINTF_WARN("Unable to dump message with topic %s"
                ". No message received.\n", topic_name_.c_str());
    return false;
  }
 ...
 private:
  ...
  bool FeedFile(const std::string& message_file, IdentifierType<T>) {
    D data;
    if (!common::util::GetProtoFromFile(message_file, &data)) {
      PRINTF_ERROR("Unable to parse input pb file %s\n", message_file.c_str());
      return false;
    }
    FeedData(data);
    return true;
  ...
  bool DumpMessage(const enable_if_t<std::is_base_of<google::protobuf::Message,
    ...
    auto header_descriptor = descriptor->FindFieldByName("header");
    if (header_descriptor == nullptr) {
      PRINTF_TRACE("Fail to find header field in pb.\n");
      return false;
    }
    ...
    if (seq_num_descriptor == nullptr) {
      PRINTF_TRACE("Fail to find sequence_num field in pb.\n");
      return false;
    }
    uint32_t sequence_num =
        header.GetReflection()->GetUInt32(header, seq_num_descriptor);
    std::ostringstream oss;
    oss << dump_path_ << "/" << sequence_num << ".pb.txt";
    return util::SetProtoToASCIIFile(message, oss.str());
  }
...
#endif  // BUBBLEFS_UTILS_APOLLO_ADAPTERS_ADAPTER_H_