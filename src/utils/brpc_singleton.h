...
// brpc/src/butil/memory/singleton.h
...
#ifndef BUBBLEFS_UTILS_BRPC_SINGLETON_H
#define BUBBLEFS_UTILS_BRPC_SINGLETON_H
...
static const base::AtomicWord kBeingCreatedMarker = 1;
...
BASE_EXPORT base::AtomicWord WaitForInstance(base::AtomicWord* instance);
...
struct StaticMemorySingletonTraits {
  ...
  static Type* New() {
    ...
    if (base::NoBarrier_AtomicExchange(&dead_, 1))
      return NULL;
  ...
  static void Resurrect() {
    base::NoBarrier_Store(&dead_, 0);
  }

 private:
  static port::AlignedMemory<sizeof(Type), ALIGNOF(Type)> buffer_;
  // Signal the object was already deleted, so it is not revived.
  static Atomic32 dead_;
};

template <typename Type> port::AlignedMemory<sizeof(Type), ALIGNOF(Type)>
...
class Singleton {
 ...
 public:
  ...
  static Type* get() {
    ...
    AtomicWord value = base::Acquire_Load(&instance_);
    ...
    if (base::Acquire_CompareAndSwap(
          &instance_, 0, internal::kBeingCreatedMarker) == 0) {
      ...
      base::Release_Store(
          &instance_, reinterpret_cast<AtomicWord>(newval));
  ...
  static void OnExit(void* /*unused*/) {
    ...
    Traits::Delete(
        reinterpret_cast<Type*>(base::NoBarrier_Load(&instance_)));
    instance_ = 0;
  }
