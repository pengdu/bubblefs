...
extern bool FLAGS_caffe2_disable_chaining = false; // "Disable chaining logic (some latent multi-device issues).");

extern bool FLAGS_caffe2_dag_net_collect_stats = false; // "Collect time stats in DAG net");  
...
DAGNetBase::DAGNetBase(
  ...
  PANIC_ENFORCE(num_workers > 0, "Must have a positive number of workers.");
  if (num_workers == 1) {
  ...
  for (int idx = 0; idx < operator_nodes_.size(); ++idx) {
    if (operator_nodes_[idx].is_chain_start_) {
      task_timers_[idx] = mycaffe2::make_unique<Timer>();
    }
  }
...
bool DAGNetBase::DoRunAsync() {
  ...
  iter_++;
  if (!job_queue_) {
    job_queue_ = mycaffe2::make_unique<SimpleQueue<int>>();
  }
