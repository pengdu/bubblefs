...
// brpc/src/butil/lazy_instance.h
...
#ifndef BUBBLEFS_UTILS_LAZY_INSTANCE_H_
#define BUBBLEFS_UTILS_LAZY_INSTANCE_H_
...
static const AtomicWord kLazyInstanceStateCreating = 1;

// Check if instance needs to be created. If so return true otherwise
// if another thread has beat us, wait for instance to be created and
// return false.
BASE_EXPORT bool NeedsLazyInstance(AtomicWord* state);

// After creating an instance, call this to register the dtor to be called
// at program exit and to update the atomic state to hold the |new_instance|
BASE_EXPORT void CompleteLazyInstance(AtomicWord* state,
...
class LazyInstance {
 public:
  ...
  Type* Pointer() {
    ...
    // fully constructed.
    static const AtomicWord kLazyInstanceCreatedMask =
    ...
    AtomicWord value = Acquire_Load(&private_instance_);
    if (!(value & kLazyInstanceCreatedMask) &&
        internal::NeedsLazyInstance(&private_instance_)) {
      // Create the instance in the space provided by |private_buf_|.
      value = reinterpret_cast<AtomicWord>(
  ...
  bool operator==(Type* p) {
    switch (NoBarrier_Load(&private_instance_)) {
      case 0:
        return p == NULL;
  ...
  AtomicWord private_instance_;
  // Preallocated space for the Type instance.
  port::AlignedMemory<sizeof(Type), ALIGNOF(Type)> private_buf_;

 private:
  Type* instance() {
    return reinterpret_cast<Type*>(NoBarrier_Load(&private_instance_));
  }
  ...
  static void OnExit(void* lazy_instance) {
    ...
    NoBarrier_Store(&me->private_instance_, 0);
  }
};
...
#endif  // BUBBLEFS_UTILS_LAZY_INSTANCE_H_