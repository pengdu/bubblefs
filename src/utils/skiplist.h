...
#ifndef BUBBLEFS_UTILS_SKIPLIST_H_
#define BUBBLEFS_UTILS_SKIPLIST_H_
...
SkipList<Key, Comparator>::NewNode(const Key& key, int height) {
  char* mem = allocator_->AllocAligned(
...
int SkipList<Key, Comparator>::RandomHeight() {
  auto rnd = random::Random::GetTLSInstance();
...
SkipList<Key, Comparator>::SkipList(const Comparator cmp, Allocator* allocator,
    ...
    : kMaxHeight_(max_height),
      ...
      kScaledInverseBranching_((random::Random::kMaxNext + 1) / kBranching_),
      compare_(cmp),
  ...
  prev_ = reinterpret_cast<Node**>(
            allocator_->AllocAligned(sizeof(Node*) * kMaxHeight_));
...
#endif // BUBBLEFS_UTILS_SKIPLIST_H_