...
// caffe2/caffe2/core/operator.h

#ifndef BUBBLEFS_UTILS_CAFFE2_OPERATOR_H_
#define BUBBLEFS_UTILS_CAFFE2_OPERATOR_H_
...
class OperatorBase : public Observable<OperatorBase> {
 public:
  ...
  inline const T& Input(int idx) {
    PANIC_ENFORCE_LT(idx, inputs_.size());
    try {
  ...
  inline const std::vector<const Blob*>& Inputs() const { return inputs_; }
  inline const std::vector<Blob*>& Outputs() { return outputs_; }
  std::vector<TensorShape> InputTensorShapes();
  ...
  inline const OperatorDef& debug_def() const {
    PANIC_ENFORCE(has_debug_def(), "operator_def was null!");
 ...
 public:
  ...
  const Event& event() const {
    PANIC_ENFORCE(event_, "Event is disabled");
    return *event_;
  }
  ...
  const std::string& type() {
    PANIC_ENFORCE(operator_def_.get() != nullptr);
...
#endif  // BUBBLEFS_UTILS_CAFFE2_OPERATOR_H_