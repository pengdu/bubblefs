...
// KlayGE/KFL/include/KFL/ArrayRef.hpp

#ifndef BUBBLEFS_UTILS_KLAGE_KFL_ARRAYREF_H_
#define BUBBLEFS_UTILS_KLAGE_KFL_ARRAYREF_H_

#include "utils/klayge_kfl_util.h"
#include <vector>
...
namespace myklayge {
        ...
        public:
                ...
                {
                        PRINTF_CHECK(n <= this->size(), "Invalid specifier");
                        return ArrayRef<T>(this->data() + n, this->size() - n);
                }

                ArrayRef<T> Slice(uint32_t n, uint32_t m) const
                {
                        PRINTF_CHECK(n + m <= this->size(), "Invalid specifier");
                        return ArrayRef<T>(this->data() + n, m);
                }

                ArrayRef<T> DropBack(uint32_t n = 1) const
                {
                        PRINTF_CHECK(this->size() >= n, "Dropping more elements than exist");
                        return this->Slice(0, this->Size() - n);
                }

                T const & operator[](size_t index) const
                {
                        PRINTF_CHECK(index < size_, "Invalid index!");
                        return data_[index];
                }
...
#endif // BUBBLEFS_UTILS_KLAGE_KFL_ARRAYREF_H_