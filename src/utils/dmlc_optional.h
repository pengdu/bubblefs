...
#ifndef BUBBLEFS_UTILS_DMLC_OPTIONAL_H_
#define BUBBLEFS_UTILS_DMLC_OPTIONAL_H_
...
struct hash<bubblefs::mydmlc::optional<T> > {
   ...
   * \return hash code.
   */
  size_t operator()(const bubblefs::mydmlc::optional<T>& val) const {
    std::hash<bool> hash_bool;
    size_t res = hash_bool(val.has_value());
    if (val.has_value()) {
      res = bubblefs::mydmlc::HashCombine(res, val.value());
    }
    return res;
  }
};
...
#endif  // BUBBLEFS_UTILS_DMLC_OPTIONAL_H_