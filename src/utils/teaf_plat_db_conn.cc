...
// Teaf/trunk/teaf(isgw)/comm/plat_db_conn.cpp

#include "utils/teaf_plat_db_conn.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int PlatDbConn::exec_multi_query(const char* sql, std::vector<MYSQL_RES*> & result_set_list)
{
    if(conn_flag_ == 0)
    {
        int ret = connect();
...
int PlatDbConn::exec_trans(const std::vector<std::string>& sqls, int& last_insert_id, int& affected_rows)
{
    if(conn_flag_ == 0)
    {
        int ret = connect();
