...
// ps-lite/include/ps/simple_app.h

#ifndef BUBBLEFS_UTILS_PSLITE_SIMPLE_APP_H_
#define BUBBLEFS_UTILS_PSLITE_SIMPLE_APP_H_
...
class SimpleApp {
 public:
  ...
  virtual inline void set_response_handle(const Handle& response_handle) {
    PANIC_ENFORCE(response_handle, "invalid response handle");
...
inline void SimpleApp::Process(const Message& msg) {
  ...
  if (msg.meta.request) {
    ...
    PANIC_ENFORCE(response_handle_, "response_handle_ is invalid");
...
#endif  // BUBBLEFS_UTILS_PSLITE_SIMPLE_APP_H_