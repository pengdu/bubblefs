...
// ps-lite/include/ps/internal/van.h
// ps-lite/src/van.cc

#ifndef BUBBLEFS_UTILS_PSLITE_VAN_H_
#define BUBBLEFS_UTILS_PSLITE_VAN_H_
...
#include "utils/pslite_network_utils.h"
#include "utils/pslite_resender.h"
...
class Van {
 public:
  ...
  const Node& my_node() const {
    PANIC_ENFORCE(ready_, "call Start() first");
    return my_node_;
  }
  /**
   * \brief stop van
...
Van* Van::Create(const std::string& type) {
  if (type == "zmq") {
    ...
    PANIC("unsupported van type: %s", type.c_str());
    return nullptr;
  }
}
...
void Van::Start() {
  // get scheduler info
  scheduler_.hostname = std::string(Environment::Get()->find("DMLC_PS_ROOT_URI"));
  scheduler_.port     = atoi(Environment::Get()->find("DMLC_PS_ROOT_PORT"));
  ...
  if (is_scheduler_) {
    ...
    if (ip.empty()) {
      ...
      PANIC_ENFORCE(!interface.empty(), "failed to get the interface");
    }
    int port = GetAvailablePort();
    const char* pstr = Environment::Get()->find("PORT");
    if (pstr) port = atoi(pstr);
    PANIC_ENFORCE(!ip.empty(), "failed to get ip");
    PANIC_ENFORCE(port, "failed to get a port");
  ...
  PANIC_ENFORCE_NE(my_node_.port, -1);
  ...
  while (true) {
    ...
    if (ready_ && drop_rate_ > 0) {
      ...
      if (rand_r(&seed) % 100 < drop_rate_) {
        PRINTF_WARN("Drop message %s\n", msg.DebugString().c_str());
        continue;
      }
    }

    PANIC_ENFORCE_NE(recv_bytes, -1);
    ...
    if (!msg.meta.control.empty()) {
      ...
      if (ctrl.cmd == Control::TERMINATE) {
        ...
        if (msg.meta.sender == Meta::kEmpty) {
          PANIC_ENFORCE(is_scheduler_, "is_scheduler_ is False");
          PANIC_ENFORCE_EQ(ctrl.node.size(), 1);
          if (nodes.control.node.size() < num_nodes) {
            nodes.control.node.push_back(ctrl.node[0]);
          } else {
            // some node dies and restarts
            PANIC_ENFORCE(ready_, "ready_ is False");
            for (size_t i = 0; i < nodes.control.node.size() - 1; ++i) {
              ...
              if (dead_set.find(node.id) != dead_set.end() && node.role == ctrl.node[0].role) {
                ...
                //PS_VLOG(1) << "replace dead node " << node.DebugString() << " by node " << recovery_node.DebugString();
        ...
        if (is_scheduler_) {
          ...
          if (nodes.control.node.size() == num_nodes) {
            ...
            for (auto& node : nodes.control.node) {
              PANIC_ENFORCE_EQ(node.id, Node::kEmpty);
            ...
            //PS_VLOG(1) << "the scheduler is connected to " << num_workers_ << " workers and " << num_servers_ << " servers";
            ready_ = true;
          } else if (recovery_nodes.control.node.size() > 0) {
            // send back the recovery node
            PANIC_ENFORCE_EQ(recovery_nodes.control.node.size(), 1);
      ...
      PANIC_ENFORCE_NE(msg.meta.sender, Meta::kEmpty);
      PANIC_ENFORCE_NE(msg.meta.recver, Meta::kEmpty);
      PANIC_ENFORCE_NE(msg.meta.customer_id, Meta::kEmpty);
      int id = msg.meta.customer_id;
      auto* obj = Postoffice::Get()->GetCustomer(id, 5);
      PANIC_ENFORCE(obj, "timeout (5 sec) to wait App %d ready", id);
      obj->Accept(msg);
    }
  }
}
...
#endif  // BUBBLEFS_UTILS_PSLITE_VAN_H_