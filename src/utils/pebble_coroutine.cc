...
// Pebble/src/common/coroutine.cpp

#include "utils/pebble_coroutine.h"
#include <sys/epoll.h>
...
static void InitCoEnv() {
    ...
    if (pid_max != NULL) {
        ...
        PEBBLE_PLOG_ERROR("open %s failed : %s", file, strerror(errno));
    }
...
struct coroutine *
_co_new(struct schedule *S, std::function<void()>& std_func) {
    if (NULL == S) {
        assert(0);
        return NULL;
    }
...
coroutine_open(uint32_t stack_size) {
    ...
    SetCoEnv(pid, env);
    PEBBLE_PLOG_INFO("init pid %ld env %p\n", (long)pid, env);
    ...
    PEBBLE_PLOG_INFO("coroutine_open is called.");
    return S;
}
...
void coroutine_close(struct schedule *S) {
    ...
    // 遍历所有的协程，逐个释放
    std::unordered_map<int64_t, coroutine*>::iterator pos = S->co_hash_map.begin();
...
int64_t coroutine_new(struct schedule *S, std::function<void()>& std_func) {
    if (NULL == S) {
        return -1;
    }
    ...
    PEBBLE_PLOG_TRACE("coroutine %ld is created.", id);
    return id;
}
...
static void mainfunc(uint32_t low32, uint32_t hi32) {
    ...
    S->running = -1;
    PEBBLE_PLOG_TRACE("coroutine %ld is deleted.", id);
}
...
int32_t coroutine_resume(struct schedule * S, int64_t id, int32_t result) {
    ...
    std::unordered_map<int64_t, coroutine*>::iterator pos = S->co_hash_map.find(id);

    // 如果在协程哈希表中没有找到，或者找到的协程内容为空
    if (pos == S->co_hash_map.end()) {
        PEBBLE_PLOG_ERROR("coroutine %ld can't find in co_map", id);
        return kCO_COROUTINE_UNEXIST;
    }

    struct coroutine *C = pos->second;
    if (NULL == C) {
        PEBBLE_PLOG_ERROR("coroutine %ld instance is NULL", id);
        return kCO_COROUTINE_UNEXIST;
    }
    ...
    switch (status) {
        case COROUTINE_READY: {
            PEBBLE_PLOG_TRACE("coroutine %ld status is COROUTINE_READY, begin to execute...", id);
        ...
        case COROUTINE_SUSPEND: {
            PEBBLE_PLOG_TRACE("coroutine %ld status is COROUTINE_SUSPEND,"
                    "begin to resume...", id);
        ...
        default:
            PEBBLE_PLOG_DEBUG("coroutine %ld status is failed, can not to be resume...", id);
...
int32_t coroutine_yield(struct schedule * S) {
    ...
    int64_t id = S->running;
    if (id < 0) {
        PEBBLE_PLOG_ERROR("have no running coroutine, can't yield.");
    ...
    if (C->status != COROUTINE_RUNNING) {
        PEBBLE_PLOG_ERROR("coroutine %ld status is SUSPEND, can't yield again.", id);
    ...
    PEBBLE_PLOG_TRACE("coroutine %ld will be yield, swith to main loop...", id);
...
int coroutine_status(struct schedule * S, int64_t id) {
    ...
    if (id < 0 || id > S->nco) {
        PEBBLE_PLOG_DEBUG("coroutine %ld not exist", id);
        return COROUTINE_DEAD;
    }
    ...
    // 如果在协程哈希表中没有找到，或者找到的协程内容为空
    if (pos == S->co_hash_map.end()) {
        PEBBLE_PLOG_DEBUG("cann't find coroutine %ld", id);
        return COROUTINE_DEAD;
    }
...
coroutine* get_curr_coroutine(struct schedule * S) {
    ...
    std::unordered_map<int64_t, coroutine*>::iterator pos = S->co_hash_map.find(S->running);

    // 如果在协程哈希表中没有找到，或者找到的协程内容为空
    if (pos == S->co_hash_map.end()) {
        PEBBLE_PLOG_FATAL("coroutine %ld can't find in co_map", S->running);
        return NULL;
    }
...
int CoroutineSchedule::Close() {
    ...
    ret += task_map_.size();
    std::unordered_map<int64_t, CoroutineTask*>::iterator it;
...
CoroutineTask* CoroutineSchedule::Find(int64_t id) const {
    ...
    std::unordered_map<int64_t, CoroutineTask*>::const_iterator it = task_map_.find(id);
    if (it != task_map_.end())
        ret = it->second;
    return ret;
}
...
int32_t CoroutineSchedule::Yield(int32_t timeout_ms) {
    ...
    if (timer_ && timeout_ms > 0) {
        ...
        timerid = timer_->StartTimer(timeout_ms,
            std::bind(&CoroutineSchedule::OnTimeout, this, co_id));
        if (timerid < 0) {
...
static unsigned long long counter(void)
{
    uint32_t lo, hi; // register 
    unsigned long long o; // register 
