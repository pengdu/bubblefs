...
// apollo/modules/common/math/line_segment2d.cc
...
double LineSegment2d::DistanceTo(const Vec2d &point,
  ...
  PANIC_ENFORCE_NE(nearest_pt, NULL);
  if (length_ <= kMathEpsilon) {
    *nearest_pt = start_;
    return point.DistanceTo(start_);
  }
...
double LineSegment2d::GetPerpendicularFoot(const Vec2d &point,
  ...
  PANIC_ENFORCE_NE(foot_point, NULL);
  if (length_ <= kMathEpsilon) {
    *foot_point = start_;
    return point.DistanceTo(start_);
  }
...
std::string LineSegment2d::DebugString() const {
  ...
  oss << "segment2d ( start = " << start_.DebugString() << "  end = "
      << end_.DebugString() << " )";
  return oss.str();
}
