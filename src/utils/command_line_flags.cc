...
// tensorflow/tensorflow/core/util/command_line_flags.cc

#include "utils/command_line_flags.h"
#include <string>
#include <vector>
...
bool ParseInt64Flag(StringPiece arg, StringPiece flag,
  ...
  *value_parsing_ok = true;
  if (arg.Consume("--") && arg.Consume(flag) && arg.Consume("=")) {
    char extra;
    int64 parsed_int64;
    if (sscanf(arg.data(), PRId64_FORMAT "%c", &parsed_int64, &extra) != 1) {
      LOG(ERROR) << "Couldn't interpret value " << arg << " for flag " << flag
                 << ".";
      *value_parsing_ok = false;
    } else {
...
Flag::Flag(const char* name, int32* dst, const string& usage_text)
    : name_(name),
      type_(TYPE_INT32),
...
Flag::Flag(const char* name, int64* dst, const string& usage_text)
    : name_(name),
      type_(TYPE_INT64),
