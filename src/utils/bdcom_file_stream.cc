...
// tera/src/common/file/file_stream.cc
...
static int32 FLAGS_file_op_retry_times = 3; // "the max retry times when file operation occurred error"
...
bool FileStream::Close(FileErrorCode* error_code) {
    ...
    if (fclose(fp_) != 0) {
        PRINTF_ERROR("fail to close file, errno: %s\n", strerror(errno));
...
int64_t FileStream::Read(void* buffer, int64_t buffer_size,
    ...
    if (success) {
        ...
        PRINTF_ERROR("error occurred in reader, errono: %s\n", strerror(errno));
...
int64_t FileStream::Write(const void* buffer, int64_t buffer_size,
    ...
    for (int32_t retry = 0; retry < FLAGS_file_op_retry_times; ++retry) {
        ...
        if (ret_size < expect_size) {
            PRINTF_ERROR("write down enough bytes and fail, ["
                "buffur_size = " PRId64_FORMAT
                ", writen down total size = " PRId64_FORMAT
                ", expect_size = %d"
                ", this writen size = %d"
                "], reason: %s\n",
                buffer_size, total_size, (int)expect_size, (int)ret_size, strerror(errno));
            PANIC_ENFORCE(ferror(fp_), "file writer is broken");
    ...
    if (total_size == buffer_size) {
        ...
        PRINTF_ERROR("error occurred in writter, errono: %s\n", strerror(errno));
...
int64_t FileStream::GetSize(const std::string& file_path,
    ...
    if (file_exist != 0) {
        ...
        if (error_code == NULL) {
            PRINTF_ERROR("file %s not exists.\n", file_path.c_str());
        }
        return file_size;
    }
...
//         SetErrorCode(error_code, errno);
    ...
    if (S_ISDIR(stat_buf.st_mode)) {
        file_size = -1;
//         SetErrorCode(error_code, errno);
        PRINTF_ERROR("input file name %s is a directory\n", file_path.c_str());
        return file_size;
    } else {
        SetErrorCode(error_code, kFileSuccess);
