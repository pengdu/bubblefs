...
// Pebble/src/common/coroutine.h

#ifndef BUBBLEFS_UTILS_PEBBLE_COROUTINE_H_
#define BUBBLEFS_UTILS_PEBBLE_COROUTINE_H_
...
constexpr int COROUTINE_DEAD = 0;
constexpr int COROUTINE_READY = 1;
constexpr int COROUTINE_RUNNING = 2;
constexpr int COROUTINE_SUSPEND = 3;

constexpr int MAX_FREE_CO_NUM = 1024;
constexpr int INVALID_CO_ID = -1;
...
struct schedule {
    ...
    std::unordered_map<int64_t, coroutine*> co_hash_map;
...
/// @return 协程ID
/// @note 只能够在主线程调用
int64_t coroutine_new(struct schedule *, std::function<void()>& std_func);
...
private:
    ...
    Timer* timer_;
    std::unordered_map<int64_t, CoroutineTask*> task_map_;
