...
// toft/system/process/sub_process.cpp

#include "platform/toft_system_process_sub_process.h"
...
#include "utils/toft_encoding_shell.h"
...
bool SubProcess::Create(const std::string& cmdline, const CreateOptions& options)
{
    ...
    if (options.m_shell) {
        ...
        if (!SplitCommandLine(cmdline, &args)) {
            PRINTF_ERROR("Can't Parse command: %s\n", cmdline.c_str());
            return false;
        }
                        ...
                        int pipe_write_fd)
{
    ...
    int error = errno;
    PRINTF_ERROR("Failed to execvpe %s\n", args[0]);

    // Notify parent process.
    if (write(pipe_write_fd, &error, sizeof(error)) != sizeof(error)) {
        PRINTF_ERROR("Write to pipe error\n");
    }
    close(pipe_write_fd);
    _Exit(127);
}
...
{
    if (m_pid > 0) {
      PRINTF_ERROR("Already running or not waited\n");
      return false;
    }

    int fds[2]; // To pass the error of execvpe.
    if (pipe(fds) < 0) {
        PRINTF_ERROR("Can't create pipe\n");
        return false;
    }
    ...
    pid_t pid = fork();
    if (pid < 0) {
      PANIC("Fork child process %s error\n", args[0]);
    }
    if (pid == 0) {
        close(pipe_read_fd);
        ...
        close(pipe_write_fd);
        PRINTF_INFO("Start process %s, pid %d\n", args[0], (int)pid);
        int error = 0;
        int nread = read(pipe_read_fd, &error, sizeof(error));
        close(pipe_read_fd);
        if (nread < 0) {
            PRINTF_ERROR("Error reading from pipe_read_fd");
            return false;
        }
        if (nread == sizeof(error)) {
            PRINTF_ERROR("Start process %s error\n", args[0]);
            MYTOFT_EINTR_IGNORED(waitpid(pid, NULL, 0));
            return false;
        }
        m_pid = pid;
        m_exit_code = 0;
...
bool SubProcess::Terminate()
{
    ...
    if (kill(m_pid, SIGTERM) < 0) {
        PRINTF_ERROR("Unable to terminate process: %d\n", (int)m_pid);
        return false;
    }
...
bool SubProcess::WaitForExit()
{
    ...
    int status;
    if (MYTOFT_EINTR_IGNORED(waitpid(m_pid, &status, 0)) == -1) {
        PRINTF_ERROR("Wait for exit failure: %d\n", (int)m_pid);
        return false;
    }
...
bool SubProcess::TimedWaitForExit(int64_t timeout_milliseconds)
{
    ...
    for (;;) {
        int status = -1;
        pid_t pid = MYTOFT_EINTR_IGNORED(waitpid(m_pid, &status, WNOHANG));
        if (pid < 0) {
            PRINTF_ERROR("Error waiting pid: %d\n", (int)m_pid);
            return false;
        }
        if (pid > 0)
            return UpdateExitStatus(status);
