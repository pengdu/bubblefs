...
// toft/system/threading/base_thread.cpp

#include "platform/toft_system_threading_base_thread.h"
...
#include "platform/toft_system_atomic_atomic.h"
...
{
    MYTOFT_CHECK_PTHREAD_ERROR(DoStart());
}
...
bool BaseThread::TryStart()
{
    ...
    if (error == 0)
        return true;
    if (error != EAGAIN)
        MYTOFT_CHECK_PTHREAD_ERROR(error);
    return false;
}

bool BaseThread::DoDetach()
{
    MYTOFT_CHECK_PTHREAD_ERROR(pthread_detach(m_handle));
    m_handle = HandleType();
    m_id = -1;
    return true;
}
...
{
    assert(IsJoinable());
    MYTOFT_CHECK_PTHREAD_ERROR(pthread_join(m_handle, NULL));
    m_handle = pthread_t();
    m_id = -1;
    return true;
}
