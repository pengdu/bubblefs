...
// toft/system/threading/semaphore.cpp

#include "platform/toft_system_threading_semaphore.h"
...
{
    if (MYTOFT_EINTR_IGNORED(sem_trywait(sem) == 0))
        return true;
    int error = errno;
    if (error == EAGAIN)
        return false;
    MYTOFT_CHECK_ERRNO_ERROR(error);
    return true;
}
...
bool SemaphoreOps::TimedAcquire(sem_t* sem, int64_t timeout) // in ms
{
    ...
    return MYTOFT_CHECK_POSIX_TIMED_ERROR(
        MYTOFT_EINTR_IGNORED(sem_timedwait(sem, &ts)));
}
...
bool NamedSemaphore::Close()
{
    if (m_sem)
    {
        MYTOFT_CHECK_POSIX_ERROR(sem_close(m_sem));
        m_sem = NULL;
        return true;
    }
    return false;
