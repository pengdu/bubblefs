...
// Pebble/src/common/log.h

#ifndef BUBBLEFS_PLATFORM_PEBBLE_LOG_H_
#define BUBBLEFS_PLATFORM_PEBBLE_LOG_H_
...
typedef std::function<void(int priority, const char* file, uint32_t line,
...
#define PEBBLE_PLOG_IF_ERROR(condition, fmt, ...) if (condition) { PEBBLE_PLOG_ERROR(fmt, ##__VA_ARGS__); }
#define PEBBLE_PLOG_IF_INFO(condition, fmt,  ...) if (condition) { PEBBLE_PLOG_INFO(fmt,  ##__VA_ARGS__); }
#define PEBBLE_PLOG_IF_DEBUG(condition, fmt, ...) if (condition) { PEBBLE_PLOG_DEBUG(fmt, ##__VA_ARGS__); }
#define PEBBLE_PLOG_IF_TRACE(condition, fmt, ...) if (condition) { PEBBLE_PLOG_TRACE(fmt, ##__VA_ARGS__); }

// 记录日志并返回
#define PEBBLE_RETURN_IF_FATAL(condition, ret, fmt, ...) if (condition) { PEBBLE_PLOG_FATAL(fmt, ##__VA_ARGS__); return (ret); }
#define PEBBLE_RETURN_IF_ERROR(condition, ret, fmt, ...) if (condition) { PEBBLE_PLOG_ERROR(fmt, ##__VA_ARGS__); return (ret); }
#define PEBBLE_RETURN_IF_INFO(condition, ret, fmt,  ...) if (condition) { PEBBLE_PLOG_INFO(fmt,  ##__VA_ARGS__); return (ret); }
#define PEBBLE_RETURN_IF_DEBUG(condition, ret, fmt, ...) if (condition) { PEBBLE_PLOG_DEBUG(fmt, ##__VA_ARGS__); return (ret); }
#define PEBBLE_RETURN_IF_TRACE(condition, ret, fmt, ...) if (condition) { PEBBLE_PLOG_TRACE(fmt, ##__VA_ARGS__); return (ret); }

// 频率控制
#define PEBBLE_LOG_VARNAME(prefix, line) PEBBLE_LOG_VARNAME_CONCAT(prefix, line)
#define PEBBLE_LOG_VARNAME_CONCAT(prefix, line) prefix ## line

#define PEBBLE_LOG_CNT_VAR     PEBBLE_LOG_VARNAME(s_log_cnt_, __LINE__)
#define PEBBLE_START_TIME_VAR  PEBBLE_LOG_VARNAME(s_start_time_ ,__LINE__)
#define PEBBLE_NOW_TIME_VAR    PEBBLE_LOG_VARNAME(s_now_time_ ,__LINE__)
