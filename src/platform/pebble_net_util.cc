...
#define INFO(fmt, ...) \
    do {\
        if (net_log_level > PEBBLE_NET_LOG_INFO_LEVEL)\
            break;\
        timeval log_now;\
        tm log_tm;\
        gettimeofday(&log_now, NULL);\
...
#define ERR(fmt, ...) \
    do {\
        if (net_log_level > PEBBLE_NET_LOG_ERROR_LEVEL)\
            break;\
        timeval log_now;\
        tm log_tm;\
        gettimeofday(&log_now, NULL);\
    ...
    #define DBG(fmt, ...)
    #define INFO(fmt, ...)
    #define ERR(fmt, ...) PEBBLE_LOG_MESSAGE((m_last_error), (sizeof(m_last_error)), fmt, ##__VA_ARGS__)
#endif
