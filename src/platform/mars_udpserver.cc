...
// mars/mars/comm/socket/udpserver.cc

#include "platform/mars_udpserver.h"
#include <functional>
#include "platform/macros.h"
#include "platform/mars_socket_address.h"
#include "platform/mars_udpclient.h"
...
void UdpServer::SendAsync(struct sockaddr_in* _addr, void* _buf, size_t _len) {
    ASSERT(fd_socket_ != INVALID_SOCKET && event_ != NULL); //"socket invalid");
...
void UdpServer::__RunLoop() {
    ASSERT(fd_socket_ != INVALID_SOCKET); //"socket invalid");
...
int UdpServer::__DoSelect(bool _bReadSet, bool _bWriteSet, void* _buf, size_t _len, struct sockaddr_in* _addr, int& _errno) {
    ASSERT(!(_bReadSet && _bWriteSet) && (_bReadSet || _bWriteSet)); // "only read or write can be true, not both");
