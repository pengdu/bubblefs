...
// mars/mars/comm/unix/thread/thread.h

#ifndef BUBBLEFS_PLATFORM_MARS_THREAD_H_
#define BUBBLEFS_PLATFORM_MARS_THREAD_H_
...
class ThreadUtil {
  public:
    ...
    static int join(thread_tid _id) {
        ...
        ASSERT(0 == ret || ESRCH == ret); // "pthread_join err:%d", ret);
        return ret;
    }
};
  ...
  public:
    ...
    int join() const {
        ...
        if (isruning()) {
            ...
            PANIC_ENFORCE(0 == ret || ESRCH == ret, "pthread_join err");
        }
...
#endif // BUBBLEFS_PLATFORM_MARS_THREAD_H_ 