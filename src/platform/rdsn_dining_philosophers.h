...
// cpp11-on-multicore/common/diningphilosophers.h

#ifndef BUBBLEFS_PLATFORM_DINING_PHILOSOPHERS_H_
#define BUBBLEFS_PLATFORM_DINING_PHILOSOPHERS_H_
...
private:
    ...
    {
        // Should not already have the target status.
        assert(allStatus.philos[philoIndex] != static_cast<size_t>(target));
        if (allStatus.philos[philoIndex] == static_cast<size_t>(target + 1))
        {
            // Decrementing this status will bring it to target.
            // Make sure the next neighbor doesn't prevent it.
            int n = neighbor(philoIndex, step);
            assert(allStatus.philos[n] != static_cast<size_t>(target + 1));    // No two neighbors should have equal status > 0.
            if (allStatus.philos[n] != static_cast<size_t>(target))
            {
                // Decrement it.
