...
// toft/system/net/domain_resolver_test.cpp

#include "platform/toft_system_net_domain_resolver.h"
...
{
    HostEntry host_entry1;
    HostEntry host_entry2;
    HostEntry host_entry3;
    int error;
    if (DomainResolver::Query("www.qq.com", &host_entry1, &error))
    {
...
TEST(DomainResolver, ResolveIpAddress)
{
    ...
    int error;
    if (DomainResolver::ResolveIpAddress("www.qq.com", &addresses, &error))
    {
        std::cout << "Query success:\nIP:";
...
TEST(DomainResolver, Invalid)
{
    std::vector<IpAddress> addresses;
    int error;
    EXPECT_FALSE(DomainResolver::ResolveIpAddress("non-exist.domaon...", &addresses, &error));
    EXPECT_NE(0, error);
    EXPECT_TRUE(DomainResolver::IsFatalError(error));
    EXPECT_EQ("Unknown host", DomainResolver::ErrorString(error));
}
