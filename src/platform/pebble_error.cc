...
struct ErrorInfo {
    ...
    std::unordered_map<int32_t, const char*> _error_info;
};
...
const char* GetErrorString(int32_t error_code) {
    ...
    std::unordered_map<int32_t, const char*>::iterator it = g_error_string_map->find(error_code);
