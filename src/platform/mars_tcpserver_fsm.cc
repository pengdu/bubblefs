...
// mars/mars/comm/socket/tcpserver_fsm.cc

#include "platform/mars_tcpserver_fsm.h"
...
TcpServerFSM::TcpServerFSM(SOCKET _socket)
    ...
    ASSERT(0 > getpeername(sock_, (sockaddr*)&addr_, &addr_len)); //TSF"getpeername:%_, %_", socket_errno, socket_strerror(socket_errno));
...
TcpServerFSM::~TcpServerFSM() {
    Close(false);
    ASSERT(INVALID_SOCKET == sock_); //"%d", sock_);
}
...
TcpServerFSM::TSocketStatus TcpServerFSM::PreReadWriteSelect(SocketSelect& _sel, XLogger& _log) {
    ASSERT(kReadWrite == status_); // "%d", status_);
