...
#ifndef BUBBLEFS_PLATFORM_PADDLE_THREADLOCAL_H_
#define BUBBLEFS_PLATFORM_PADDLE_THREADLOCAL_H_
...
public:
  ThreadLocal() {
    PANIC_ENFORCE_EQ(pthread_key_create(&threadSpecificKey_, dataDestructor), 0);
  }
  ...
  void set(T* p) {
    ...
    PANIC_ENFORCE_EQ(pthread_setspecific(threadSpecificKey_, p), 0);
  }
...
class ThreadLocalD {
public:
  ThreadLocalD() { PANIC_ENFORCE_EQ(pthread_key_create(&threadSpecificKey_, NULL), 0); }
  ~ThreadLocalD() {
...
#endif // BUBBLEFS_PLATFORM_PADDLE_THREADLOCAL_H_