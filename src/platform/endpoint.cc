...
#include "platform/endpoint.h"                    // ip_t
...
int tcp_connect(EndPoint point, int* self_port) {
    base::fd_guard sockfd(socket(AF_INET, SOCK_STREAM, 0));
    if (sockfd < 0) {
        return -1;
    }
    ...
    if (self_port != NULL) {
        ...
        if (get_local_side(sockfd, &pt) == 0) {
          ...
          fprintf(stderr, "Fail to get the local port of sockfd\n");
          abort();
        }
    }
