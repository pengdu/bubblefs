...
#ifndef BUBBLEFS_PLATFORM_DMLC_LOGGING_H_
#define BUBBLEFS_PLATFORM_DMLC_LOGGING_H_
...
#define CHECK_BINARY_OP(name, op, x, y)                               \
  if (mydmlc::LogCheckError _check_err = mydmlc::LogCheck##name(x, y))    \
    mydmlc::LogMessageFatal(__FILE__, __LINE__).stream()                \
...
#define CHECK_NOTNULL(x) \
  ((x) == NULL ? mydmlc::LogMessageFatal(__FILE__, __LINE__).stream() << "Check  notnull: "  #x << ' ', (x) : (x)) // NOLINT(*)
// Debug-only checking.
#ifdef NDEBUG
#define DCHECK(x) \
  while (false) CHECK(x)
...
#define LOG_INFO mydmlc::CustomLogMessage(__FILE__, __LINE__)
#else
#define LOG_INFO mydmlc::LogMessage(__FILE__, __LINE__)
#endif
#define LOG_ERROR LOG_INFO
#define LOG_WARNING LOG_INFO
#define LOG_FATAL mydmlc::LogMessageFatal(__FILE__, __LINE__)
...
#define LOG_IF(severity, condition) \
  !(condition) ? (void)0 : mydmlc::LogMessageVoidify() & LOG(severity)

#ifdef NDEBUG
#define LOG_DFATAL LOG_ERROR
#define DFATAL ERROR
#define DLOG(severity) true ? (void)0 : mydmlc::LogMessageVoidify() & LOG(severity)
#define DLOG_IF(severity, condition) \
  (true || !(condition)) ? (void)0 : mydmlc::LogMessageVoidify() & LOG(severity)
#else
...
inline std::string StackTrace() {
  ...
  if (msgs != nullptr) {
    for (int frameno = 0; frameno < nframes; ++frameno) {
      string msg = mydmlc::Demangle(msgs[frameno]);
...
#endif
#endif  // BUBBLEFS_PLATFORM_DMLC_LOGGING_H_