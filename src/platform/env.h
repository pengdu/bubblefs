...
class EnvWrapper : public Env {
 public:
  ...
  Status GetChildrenFileAttributes(
      const string& dir, std::vector<FileAttributes>* result) override {
  ...
  Status CreateDirIfMissing(const string& d) override {
    return target_->CreateDirIfMissing(d);
  }
  Status DeleteDir(const string& d) override {
    return target_->DeleteDir(d);
  }
  Status GetFileSize(const string& f, uint64_t* s) override {
    return target_->GetFileSize(f, s);
  }

  Status GetFileModificationTime(const string& fname,
  ...
  Status LinkFile(const string& s, const string& t) override {
    return target_->LinkFile(s, t);
  }

  Status LockFile(const string& f, FileLock** l) override {
