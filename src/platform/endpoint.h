...
#ifndef BUBBLEFS_PLATFORM_ENDPOINT_H_
#define BUBBLEFS_PLATFORM_ENDPOINT_H_
...
inline bool operator<(netutil::ip_t lhs, netutil::ip_t rhs) {
    return netutil::ip2int(lhs) < netutil::ip2int(rhs);
}
inline bool operator>(netutil::ip_t lhs, netutil::ip_t rhs) {
    return rhs < lhs;
}
...
inline std::ostream& operator<<(std::ostream& os, const netutil::IPStr& ip_str) {
    return os << ip_str.c_str();
}
inline std::ostream& operator<<(std::ostream& os, netutil::ip_t ip) {
    return os << netutil::ip2str(ip);
}
...
template <>
struct hash<netutil::EndPoint> {
    std::size_t operator()(const netutil::EndPoint& ep) const {
        return HashPair(netutil::ip2int(ep.ip), ep.port);
    }
};

} // namespace bubblefs

#endif // BUBBLEFS_PLATFORM_ENDPOINT_H_